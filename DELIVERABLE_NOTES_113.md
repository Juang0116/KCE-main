...
- Added `LaunchTrustRail` and integrated it into `/plan`, `/contact`, `/checkout/success` and
  `/booking/[session_id]`.
...
4. Verify `/apple-touch-icon.png`, `/safari-pinned-tab.svg`, `/manifest.webmanifest`,
   `/site.webmanifest`, `/humans.txt`, `/.well-known/security.txt`
