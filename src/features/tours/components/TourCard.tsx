...
export default function TourCard({
  ...
  return (
    <article className="group flex flex-col overflow-hidden rounded-[2rem] border border-[color:var(--color-border)] bg-[color:var(--color-surface)] shadow-soft transition-all duration-300 hover:-translate-y-1 hover:border-brand-blue/30 hover:shadow-pop">
      ...
      <div className="flex flex-1 flex-col p-6">
        <div className="mb-2 flex items-start justify-between gap-4">
          <Link
            href={link}
            className="z-10 no-underline hover:no-underline"
          >
            <h3 className="line-clamp-2 font-heading text-xl leading-tight text-[color:var(--color-text)] transition-colors group-hover:text-brand-blue">
              {tour.title}
            </h3>
          </Link>
        </div>

        <p className="text-[color:var(--color-text)]/70 mt-2 line-clamp-2 flex-1 text-sm font-light">
          ...
          <div>
            ...
            <p className="font-heading text-2xl text-brand-blue">
              {tour.base_price != null
                ? formatCurrencyEUR(tour.base_price)
                : tour.price != null
                  ? formatCurrencyEUR(tour.price)
                  : '—'}
            </p>
          </div>
          <Link
            href={link}
            className="z-10 inline-flex h-10 w-10 items-center justify-center rounded-full bg-brand-blue/10 text-brand-blue transition-colors group-hover:bg-brand-blue group-hover:text-white"
          >
