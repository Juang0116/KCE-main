    ...
    <div className="rounded-xl border border-[color:var(--color-border)] bg-[color:var(--color-surface-2)] p-3">
      ...
      <div className="mt-2">
        ...
        <button
          ...
          className="flex w-full items-center justify-between rounded-xl border border-[color:var(--color-border)] px-3 py-2 text-sm outline-none hover:border-brand-blue/50 bg-[color:var(--color-surface)] transition-colors"
        >
        ...
        {showCalendar && (
          <div className="fixed inset-0 z-[99999] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-in fade-in">
            <div className="bg-white dark:bg-slate-900 border border-[color:var(--color-border)] rounded-3xl p-6 shadow-2xl flex flex-col items-center w-full max-w-sm">
              <div className="w-full flex justify-between items-center mb-4 pb-4 border-b border-gray-100 dark:border-white/10">
                <h4 className="font-bold text-[color:var(--color-text)] text-sm">Selecciona tus fechas</h4>
                <button type="button" onClick={() => setShowCalendar(false)} className="p-2 hover:bg-gray-100 rounded-full">
                  <X className="size-4" />
                </button>
              </div>
              <DayPicker
              ...
              <div className="w-full mt-6 flex justify-end gap-3">
                <Button variant="outline" onClick={() => onDateChange(item.slug, undefined)} className="h-10 text-xs rounded-xl">Limpiar</Button>
                <Button onClick={() => setShowCalendar(false)} className="bg-brand-blue text-white h-10 text-xs rounded-xl px-6">Confirmar</Button>
              </div>
            </div>
          </div>
        )}
...
export default function BookingWidget({
  ...
  const refreshIdentity = React.useCallback(async () => {
    ...
    if (user) {
      const { data } = await (supabase as any).from('customers').select('identity_status').eq('id', user.id).maybeSingle();
      setIdentityStatus(data?.identity_status || 'none');
    }
  }, [supabase]);

  React.useEffect(() => { refreshIdentity(); }, [refreshIdentity]);
  ...
  const waHref = React.useMemo(() => {
    ...
    if (!number) return '';
    const tourLines = items.map((it, i) => {
      const dates = it.dateRange?.from ? `${format(it.dateRange.from, 'dd/MM')} al ${it.dateRange.to ? format(it.dateRange.to, 'dd/MM') : '...'}` : 'Sin definir';
      return `Tour ${i + 1}: ${it.title} — Fechas: ${dates}`;
    }).join('\n');
    const msg = [`Hola KCE, quiero información.`, '', tourLines, '', `Personas: ${qtyNumber}`, '', '¿Me confirmas disponibilidad?'].join('\n');
    return buildWhatsAppHref({ number, message: msg, url: `${pathname ?? ''}${sp?.toString() ? `?${sp.toString()}` : ''}` });
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    for (const it of items) {
      if (!it.dateRange?.from || !it.dateRange?.to) return setError(`Selecciona fechas para "${it.title}".`);
    }
    ...
    try {
      const body = isCombo ? {
        mode: 'combo',
        items: items.map(it => ({ slug: it.slug, start_date: it.dateRange!.from!.toISOString(), end_date: it.dateRange!.to!.toISOString() })),
        guests: qtyNumber, customer: { name: name.trim(), email: email.trim() }, currency: 'EUR'
      } : {
        tour: { slug: items[0]!.slug, title: items[0]!.title, short },
        quantity: qtyNumber, customer: { name: name.trim(), email: email.trim() },
        start_date: items[0]!.dateRange!.from!.toISOString(), end_date: items[0]!.dateRange!.to!.toISOString(), currency: 'EUR'
      };
  ...
  return (
    <aside className="rounded-2xl border border-[color:var(--color-border)] bg-[color:var(--color-surface)] p-5 shadow-soft">
      <div className="flex items-end justify-between mb-4">
        <div>
          <div className="text-[color:var(--color-text-muted)] text-xs">{isCombo ? 'Combo' : 'Desde'}</div>
          <div className="font-heading text-2xl text-brand-red">{fmtEUR(pricing.total)}</div>
          <div className="text-[color:var(--color-text-muted)] text-xs mt-0.5">total · {qtyNumber} {qtyNumber === 1 ? 'persona' : 'personas'}</div>
        </div>
        {isCombo && (
          <div className="flex items-center gap-1.5 rounded-full bg-green-500/10 border border-green-500/20 px-3 py-1">
            ...
            <span className="text-[11px] font-bold text-green-700">−{Math.round((COMBO_DISCOUNTS[items.length] ?? 0) * 100)}% combo</span>
          </div>
        )}
      </div>
      ...
      <form onSubmit={onSubmit} className="space-y-3" noValidate>
        <div className="space-y-2">
          {items.map((item, i) => (
            <ComboTourRow key={item.slug} item={item} index={i} qty={qtyNumber} minSelectableDate={minSelectableDate} onRemove={removeTour} onDateChange={updateDateRange} />
          ))}
        </div>
        ...
        {addableTours.length > 0 && items.length < 3 && (
          <div className="rounded-xl border border-dashed border-brand-blue/30 bg-brand-blue/5 p-3">
            <p className="text-xs font-semibold text-brand-blue mb-2 flex items-center gap-1"><Plus className="size-3" /> Agregar tour al combo</p>
            <div className="flex flex-col gap-1.5">
              {addableTours.map((t) => (
                <button key={t.slug} type="button" onClick={() => addTour(t)} className="w-full text-left rounded-xl border border-[color:var(--color-border)] bg-[color:var(--color-surface)] px-3 py-2 text-sm hover:border-brand-blue/50 transition-colors">
        ...
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 mt-4">
          <label className="text-[color:var(--color-text)] block text-sm font-medium">Nombre completo
            <input required value={name} onChange={(e) => setName(e.currentTarget.value)} className="mt-1 w-full rounded-xl border border-[color:var(--color-border)] px-3 py-2 bg-[color:var(--color-surface-2)]" />
          </label>
          <label className="text-[color:var(--color-text)] block text-sm font-medium">Email
            <input type="email" required value={email} onChange={(e) => setEmail(e.currentTarget.value)} className="mt-1 w-full rounded-xl border border-[color:var(--color-border)] px-3 py-2 bg-[color:var(--color-surface-2)]" />
          </label>
        </div>

        {/* ── SECCIÓN DE VERIFICACIÓN ── */}
        <div className="mt-6 pt-6 border-t border-[color:var(--color-border)] space-y-4">
          {(identityStatus === 'none' || identityStatus === 'rejected') && (
            <div className="space-y-3">
              <div className="bg-amber-50 dark:bg-amber-950/20 border border-amber-200 dark:border-amber-900/30 p-3 rounded-xl">
                <p className="text-[11px] text-amber-800 dark:text-amber-400 leading-relaxed font-medium">
                  <strong>Verificación necesaria:</strong> Requerimos una foto de tu ID o Pasaporte para procesar la reserva por seguridad.
                </p>
              </div>
          ...
          {identityStatus === 'pending' && (
            <div className="bg-blue-50 dark:bg-blue-950/20 border border-blue-200 p-3 rounded-xl flex items-center gap-3">
              <Clock className="size-4 text-blue-600" />
              <p className="text-[11px] text-blue-800">Documento en revisión. Puedes pagar, pero la reserva depende de la aprobación final.</p>
            </div>
          )}
          
          <Button
            type="submit"
            className="w-full bg-brand-blue text-white hover:bg-brand-blue/90 h-12"
            disabled={pending || identityStatus === 'none' || identityStatus === 'rejected'}
          >
            {pending ? <Loader2 className="size-4 animate-spin" /> : `Ir al pago seguro (${fmtEUR(pricing.total)})`}
          </Button>
        </div>

        <OpenChatButton variant="outline" className="w-full mt-2" addQueryParam>Hablar con nuestra IA</OpenChatButton>
        
        {waHref && (
          <a href={waHref} target="_blank" rel="noopener noreferrer" className="inline-flex w-full items-center justify-center rounded-xl border border-[color:var(--color-border)] px-4 py-2 text-sm font-medium mt-2">
