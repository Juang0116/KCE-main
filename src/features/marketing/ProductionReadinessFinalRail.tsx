...
function copy(
  locale: SupportedLocale,
  variant: Variant,
): { eyebrow: string; title: string; description: string; note: string; items: Item[] } {
      ...
      items: [
        {
          ...
          body:
            locale === 'es'
              ? 'La reserva sigue siendo la verdad operativa después del pago.'
              : 'The booking remains the operational truth after payment.',
        ...
        },
        {
          ...
          body:
            locale === 'es'
              ? 'Cuenta sirve para reentrada, activos y continuidad, no solo para login.'
              : 'Account is for re-entry, assets and continuity, not only login.',
        ...
        },
        {
          ...
          body:
            locale === 'es'
              ? 'Soporte trabaja mejor cuando hereda booking, ticket y conversación.'
              : 'Support works best when it inherits booking, ticket and conversation.',
        ...
        },
        {
          ...
          body:
            locale === 'es'
              ? 'El handoff humano debe quedarse como escalamiento premium, no como canal paralelo.'
              : 'Human handoff should remain a premium escalation, not a parallel channel.',
          href: withLocale(locale, '/contact?source=production-readiness-postpurchase'),
        },
      ],
    };
  }
      ...
      items: [
        {
          ...
          body:
            locale === 'es'
              ? 'Continúa el caso en un único hilo siempre que sea posible.'
              : 'Continue the case in one thread whenever possible.',
          href: withLocale(locale, '/account/support'),
        },
        {
          title: l.bookings,
          body:
            locale === 'es'
              ? 'Confirma fechas, invoice y logística desde reservas antes de escalar.'
              : 'Confirm dates, invoice and logistics from bookings before escalating.',
          href: withLocale(locale, '/account/bookings'),
        },
        {
          title: l.account,
          body:
            locale === 'es'
              ? 'Cuenta conserva la base estable para volver sin perder continuidad.'
              : 'Account preserves the stable base to return without losing continuity.',
          href: withLocale(locale, '/account'),
        },
        {
          title: l.contact,
          body:
            locale === 'es'
              ? 'Escala a contacto premium cuando el caso sí exige un handoff directo.'
              : 'Escalate to premium contact when the case truly requires direct handoff.',
          href: withLocale(locale, '/contact?source=production-readiness-support'),
        },
      ],
    };
  }
      ...
      items: [
        {
          ...
          body:
            locale === 'es'
              ? 'Reserva, invoice y calendario tienen que seguir a un toque de distancia.'
              : 'Booking, invoice and calendar should remain one tap away.',
          href: withLocale(locale, '/account/bookings'),
        },
        {
          title: l.support,
          body:
            locale === 'es'
              ? 'Soporte hereda contexto y evita abrir casos desconectados.'
              : 'Support inherits context and avoids disconnected cases.',
          href: withLocale(locale, '/account/support'),
        },
        {
          title: l.contact,
          body:
            locale === 'es'
              ? 'Contacto queda como handoff premium cuando ya toca seguimiento humano.'
              : 'Contact stays as premium handoff when human follow-up is needed.',
          href: withLocale(locale, '/contact?source=production-readiness-account'),
        },
        {
          title: l.tours,
          body:
            locale === 'es'
              ? 'El catálogo sigue visible para ampliar el viaje sin romper continuidad.'
              : 'The catalog remains visible to extend the trip without breaking continuity.',
    ...
    items: [
      {
        ...
        body:
          locale === 'es'
            ? 'Comparar opciones reales con menos fricción visual y más claridad comercial.'
            : 'Compare real options with less visual friction and more commercial clarity.',
        href: withLocale(locale, '/tours'),
      },
      {
        ...
        body:
          locale === 'es'
            ? 'Ordenar la ruta por ciudad o región cuando el viaje aún no está cerrado.'
            : 'Sort the route by city or region when the trip is not closed yet.',
      ...
      },
      {
        ...
        body:
          locale === 'es'
            ? 'Absorber duda, presupuesto y ritmo para convertirlos en una propuesta guiada.'
            : 'Absorb doubt, budget and pace to turn them into a guided proposal.',
      ...
      },
      {
        ...
        body:
          locale === 'es'
            ? 'Proteger el handoff humano cuando el caso ya exige seguimiento premium.'
            : 'Protect human handoff when the case already requires premium follow-up.',
        href: withLocale(locale, '/contact?source=production-readiness-public'),
      },
    ],
  };
}

export default function ProductionReadinessFinalRail({
  locale,
  variant = 'public',
  className,
}: Props) {
  ...
  return (
    <section
      className={`border-brand-blue/12 rounded-[2rem] border bg-[color:var(--color-surface)] p-6 shadow-soft ${className || ''}`}
    >
      <div className="max-w-3xl">
        ...
        <p className="text-[color:var(--color-text)]/74 mt-3 max-w-3xl text-sm leading-6 md:text-base">
          {data.description}
        </p>
      </div>
      ...
      <div className="mt-6 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        {data.items.map((item, idx) => (
          <Link
            key={`${item.title}-${idx}`}
            href={item.href}
            className="hover:border-brand-blue/18 hover:bg-brand-blue/7 group rounded-[1.35rem] border border-brand-blue/10 bg-brand-blue/5 p-4 transition hover:-translate-y-0.5"
          >
            ...
            <p className="text-[color:var(--color-text)]/74 mt-2 text-sm leading-6">{item.body}</p>
      ...
      <div className="text-[color:var(--color-text)]/72 mt-6 rounded-[1.35rem] border border-brand-blue/10 bg-white/70 px-4 py-3 text-sm leading-6 shadow-soft dark:bg-black/10">
        <span className="font-semibold text-brand-blue">
          {locale === 'es' ? 'Nota final:' : 'Final note:'}
        </span>{' '}
        {data.note}
      </div>
    </section>
