      ...
      items: [
        {
          ...
          body:
            locale === 'es'
              ? 'La reserva sigue siendo el centro operativo después del checkout.'
              : 'The booking stays as the operational center after checkout.',
        ...
        },
        {
          ...
          body:
            locale === 'es'
              ? 'Cuenta sirve para recuperar activos y continuidad, no solo para autenticarse.'
              : 'Account helps recover assets and continuity, not just authenticate.',
        ...
        },
        {
          ...
          body:
            locale === 'es'
              ? 'Soporte debe abrir un solo hilo por caso con booking, ticket y contexto ya visibles.'
              : 'Support should open one thread per case with booking, ticket and context already visible.',
        ...
        },
        {
          ...
          body:
            locale === 'es'
              ? 'Cuando toque escalar, contacto premium debe heredar el contexto y no empezar de cero.'
              : 'When escalation is needed, premium contact should inherit context instead of starting from zero.',
          href: withLocale(locale, '/contact?source=go-live-postpurchase'),
        },
      ],
    };
  }
      ...
      items: [
        {
          ...
          body:
            locale === 'es'
              ? 'Abre o continúa un único ticket por caso principal.'
        ...
        },
        {
          ...
          body:
            locale === 'es'
              ? 'Vuelve a reservas cuando haga falta confirmar fechas, invoice o logística.'
              : 'Return to bookings when dates, invoice or logistics need confirmation.',
          href: withLocale(locale, '/account/bookings'),
        },
        {
          title: l.contact,
          body:
            locale === 'es'
              ? 'Escala a contacto premium solo cuando el caso ya exige un handoff humano más directo.'
              : 'Escalate to premium contact only when the case truly needs a more direct human handoff.',
          href: withLocale(locale, '/contact?source=go-live-support'),
        },
        {
          title: l.account,
          body:
            locale === 'es'
              ? 'Mantén cuenta como base estable para no perder reentrada y continuidad.'
              : 'Keep account as the stable base to preserve re-entry and continuity.',
          href: withLocale(locale, '/account'),
        },
      ],
    };
    ...
    items: [
      {
        ...
        body:
          locale === 'es'
            ? 'Catálogo para comparar opciones reales y entrar a detalle con menos fricción.'
            : 'Catalog to compare real options and enter detail with less friction.',
      ...
      },
      {
        ...
        body:
          locale === 'es'
            ? 'Entrada por ciudad o región para viajeros que aún ordenan su ruta.'
            : 'Entry by city or region for travelers still shaping their route.',
      ...
      },
      {
        ...
        body:
          locale === 'es'
            ? 'Plan personalizado absorbe la duda y la convierte en decisión guiada.'
            : 'The personalized plan absorbs uncertainty and turns it into guided choice.',
      ...
      },
      {
        ...
        body:
          locale === 'es'
            ? 'Contacto premium protege el handoff cuando el caso ya merece ayuda humana.'
            : 'Premium contact protects the handoff when the case deserves human help.',
    ...
    >
      ...
      <div className="text-[color:var(--color-text)]/72 mt-6 rounded-[1.35rem] border border-brand-blue/10 bg-white/70 px-4 py-3 text-sm leading-6 shadow-soft dark:bg-black/10">
        <span className="font-semibold text-brand-blue">
          {locale === 'es' ? 'Nota de lanzamiento:' : 'Launch note:'}
        </span>{' '}
        {data.note}
      </div>
    </section>
