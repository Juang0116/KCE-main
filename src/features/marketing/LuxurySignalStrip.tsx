    ...
    <section className="overflow-hidden rounded-[28px] border border-[color:var(--color-border)] bg-[linear-gradient(145deg,rgba(255,255,255,0.98),rgba(248,244,236,0.97))] shadow-soft">
      <div className="grid gap-0 lg:grid-cols-[1.04fr_0.96fr]">
        <div className="p-6 md:p-8">
          <div className="border-brand-blue/12 inline-flex rounded-full border bg-[color:var(--color-surface)] px-3 py-1 text-[11px] font-semibold uppercase tracking-[0.18em] text-brand-blue shadow-soft">
            premium signal system
          </div>
          <h2 className="mt-4 font-heading text-2xl text-brand-blue md:text-3xl">
            Keep KCE feeling premium even when the traveler moves fast
          </h2>
          <p className="text-[color:var(--color-text)]/72 mt-3 max-w-3xl text-sm leading-7">
            This strip acts like a final luxury reminder: cleaner hierarchy, calmer support and a
            sharper commercial narrative all raise confidence before and after booking.
          </p>
          ...
          <div className={`mt-6 grid gap-4 ${compact ? 'md:grid-cols-3' : 'md:grid-cols-3'}`}>
            {tiles.map((tile) => (
              <article
                key={tile.title}
                className="rounded-[22px] border border-[color:var(--color-border)] bg-[color:var(--color-surface)] p-4 shadow-soft"
              >
        ...
        <aside className="border-t border-brand-blue/10 bg-[linear-gradient(160deg,rgba(8,41,86,0.98),rgba(11,84,162,0.96)_62%,rgba(216,176,74,0.34))] p-6 text-white md:p-8 lg:border-l lg:border-t-0">
          ...
          <div className="mt-4 space-y-3">
            {[
              ...
              <div
                key={item}
                className="bg-white/8 rounded-[20px] border border-white/10 p-4"
              >
          ...
          <div className="mt-5 flex flex-wrap gap-2 text-xs">
            <Link
              ...
              className="border-white/12 inline-flex items-center rounded-full border bg-white/10 px-3 py-1.5 font-semibold text-white transition hover:bg-white/15"
            >
              Browse tours
            </Link>
            <Link
