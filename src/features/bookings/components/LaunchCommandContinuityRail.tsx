    ...
    >
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          <h2 className="mt-2 font-heading text-2xl text-brand-blue md:text-3xl">
            Cuenta, booking, soporte y handoff ya deberían funcionar como un solo sistema
          </h2>
          <p className="text-[color:var(--color-text)]/72 mt-3 max-w-3xl text-sm leading-6 md:text-base">
            Esta capa final de lanzamiento busca que el viajero no tenga que adivinar a dónde ir:
            revisar reservas, abrir soporte, volver al contacto o retomar la cuenta debe sentirse
            continuo.
          </p>
        </div>
        ...
        {cards.map((card) => (
          ...
          >
            <div
              className={
                card.tone === 'dark'
                  ? 'text-[11px] uppercase tracking-[0.18em] text-white/65'
                  : 'text-[color:var(--color-text)]/52 text-[11px] uppercase tracking-[0.18em]'
              }
            >
              {card.eyebrow}
            </div>
            <h3
              className={
                card.tone === 'dark'
                  ? 'mt-4 font-heading text-2xl text-white'
                  : 'mt-4 font-heading text-xl text-brand-blue'
              }
            >
              {card.title}
            </h3>
            <p
              className={
                card.tone === 'dark'
                  ...
                  : 'text-[color:var(--color-text)]/72 mt-2 text-sm leading-6'
              }
            >
              {card.body}
            </p>
      ...
      <div className="mt-5 grid gap-3 lg:grid-cols-[1fr_1fr]">
        <div className="rounded-3xl border border-[color:var(--color-border)] bg-[color:var(--color-surface-2)] p-5">
          <p className="text-[color:var(--color-text)]/52 text-[11px] uppercase tracking-[0.18em]">
            human follow-up
          </p>
          <h3 className="mt-3 font-heading text-xl text-brand-blue">
            Si el caso requiere una persona, la transición debe ser elegante
          </h3>
          <p className="text-[color:var(--color-text)]/72 mt-2 text-sm leading-6">
            Contacto, soporte y CRM deben compartir el mismo resumen para evitar que el viajero
            repita historia, pago o referencia del tour.
          </p>
          <div className="mt-4">
            <Button
              asChild
              variant="ghost"
              className="justify-start px-0 text-brand-blue"
            >
        ...
        <div className="rounded-3xl border border-[color:var(--color-border)] bg-[color:var(--color-surface-2)] p-5">
          ...
          <ul className="text-[color:var(--color-text)]/72 mt-3 space-y-2 text-sm leading-6">
