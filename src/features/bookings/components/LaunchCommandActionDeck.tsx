  ...
  return (
    ...
    >
      <div className="max-w-3xl">
        ...
        <p className="text-[color:var(--color-text)]/72 mt-3 text-sm leading-6">{description}</p>
      </div>
      ...
      <div className="mt-5 flex gap-3 overflow-x-auto pb-1 md:hidden">
        {actions.map((action) => (
          <div
            ...
            className="min-w-[15rem] flex-1 rounded-[1.5rem] border border-[color:var(--color-border)] bg-[color:var(--color-surface-2)] p-4"
          >
            <p className="font-heading text-base text-brand-blue">{action.label}</p>
            <p className="text-[color:var(--color-text)]/70 mt-2 text-sm leading-5">
              {action.detail}
            </p>
            <div className="mt-4">
              <Button
      ...
      <div className="mt-5 hidden gap-4 md:grid md:grid-cols-2 xl:grid-cols-4">
        {actions.map((action) => (
          <div
            key={`${action.label}-${action.href}`}
            className="rounded-[1.7rem] border border-[color:var(--color-border)] bg-[color:var(--color-surface-2)] p-5"
          >
