  ...
  return (
    <div className="mt-8 rounded-2xl border border-brand-dark/5 bg-surface-2 p-6 transition-all">
      <div className="flex items-center gap-4">
        <div
          className={`flex h-10 w-10 items-center justify-center rounded-full ${status === 'sent' ? 'bg-green-100 text-green-600' : 'bg-brand-blue/5 text-brand-blue'}`}
        >
          {status === 'sending' ? (
        ...
        <div>
          ...
          <p className="text-sm font-light text-main">
            {status === 'sending'
              ? 'Generando tu factura legal y voucher...'
              : status === 'sent'
                ? 'Voucher enviado exitosamente a tu bandeja.'
      ...
      {status === 'error' && (
        <Button
          onClick={send}
          variant="ghost"
          className="mt-4 p-0 text-xs font-bold uppercase text-brand-blue"
        >
          Reintentar envío
        </Button>
      )}
