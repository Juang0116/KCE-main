...
export default function CookieConsentBanner({ dict }: Props) {
  ...
  return (
    <div className="animate-in fade-in slide-in-from-bottom-4 fixed bottom-0 left-0 right-0 z-[110] p-4 duration-500">
      <div className="mx-auto max-w-4xl overflow-hidden rounded-[2rem] border border-brand-dark/10 bg-white/95 p-6 shadow-2xl backdrop-blur-md dark:bg-brand-dark/95">
        <div className="flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
          <div className="space-y-1">
            ...
            <p className="text-[color:var(--color-text)]/70 max-w-xl text-sm leading-relaxed dark:text-white/70">
              {t(
                dict,
                'cookies.description',
                'Utilizamos cookies para mejorar tu experiencia de reserva y entender cómo exploras Colombia con nosotros.',
              )}
              <a
                href="/privacy"
                className="ml-1 font-bold text-brand-blue underline decoration-brand-blue/30 underline-offset-4 hover:text-brand-yellow"
              >
