...
const checklist = [
  [
    'QA + RC Verify',
    'Ejecuta Production Preflight y una verificación real de compra antes de declarar el Release Candidate.',
  ],
  [
    'Delivery assets',
    'Comprueba booking, invoice y calendar con un caso real. Si algo falla, usa heal/retry de inmediato.',
  ],
  [
    'Account confidence',
    'Verifica que el viajero pueda descargar activos y volver al tour sin perder el contexto de su sesión.',
  ],
  [
    'Growth safety',
    'Confirma que el funnel completo resiste tráfico internacional masivo y no solo navegación local de pruebas.',
  ],
] as const;
            ...
            {lanes.map((lane) => {
              ...
              return (
                ...
                >
                  ...
                  <Icon className="mt-4 h-5 w-5 text-brand-blue opacity-40 transition-opacity group-hover:opacity-100" />
                  <h3 className="mt-3 font-heading text-lg font-bold leading-tight text-brand-blue">
                    {lane.title}
                  </h3>
                  <p className="mt-3 text-sm leading-relaxed text-muted">{lane.body}</p>
                </article>
              );
            })}
          </div>
