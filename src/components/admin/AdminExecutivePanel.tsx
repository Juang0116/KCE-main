    ...
    <div className="space-y-8 animate-fade-in w-full max-w-[var(--container-max)] mx-auto">
      ...
      <header className="flex flex-col md:flex-row md:items-end justify-between gap-6 border-b border-[var(--color-border)] pb-8">
        <div>
          ...
          <h1 className="text-3xl md:text-4xl font-heading font-bold text-[var(--color-text)] tracking-tight">
            Executive <span className="text-brand-terra">Overview</span>
          </h1>
          <p className="text-[var(--color-text-muted)] mt-2 font-body text-sm md:text-base max-w-2xl">
            Visión global de rendimiento, conversiones y operaciones tácticas de Kolombia Coffee Experience.
          </p>
        </div>
        <div className="flex items-center gap-3">
          <button className="btn btn-outline text-xs bg-[var(--color-surface)] backdrop-blur-sm shadow-soft">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <AdminCard hoverEffect className="group">
          <AdminCardSubtitle className="flex items-center justify-between">
            Reservas Activas (Mes) <BarChart3 className="w-4 h-4 text-[var(--color-text-muted)] opacity-50 group-hover:text-brand-blue transition-colors" />
          </AdminCardSubtitle>
          <div className="mt-4 flex items-end justify-between gap-3">
            <span className="text-5xl font-heading font-bold text-[var(--color-text)] group-hover:text-brand-blue transition-colors">
              142
            </span>
            <span className="text-xs font-bold uppercase tracking-wider text-[var(--color-success)] mb-2 flex items-center bg-[var(--color-success)]/10 px-2.5 py-1 rounded-md border border-[var(--color-success)]/20">
        ...
        <AdminCard hoverEffect className="group">
          <AdminCardSubtitle className="flex items-center justify-between">
            Ingresos Proyectados <Target className="w-4 h-4 text-[var(--color-text-muted)] opacity-50 group-hover:text-brand-yellow transition-colors" />
          </AdminCardSubtitle>
          <div className="mt-4 flex items-end justify-between gap-3">
        ...
        <AdminCard hoverEffect className="group">
          <AdminCardSubtitle className="flex items-center justify-between">
            Conversión General <Activity className="w-4 h-4 text-[var(--color-text-muted)] opacity-50 group-hover:text-brand-terra transition-colors" />
          </AdminCardSubtitle>
          <div className="mt-4 flex items-end justify-between gap-3">
            <span className="text-5xl font-heading font-bold text-brand-terra">
              4.8%
            </span>
            <span className="text-xs font-bold tracking-wider text-[var(--color-text-muted)] mb-2 flex items-center bg-[var(--color-surface-2)] px-2.5 py-1 rounded-md border border-[var(--color-border)]">
              Estable
            </span>
          </div>
        </AdminCard>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-8">
        ...
        <div className="xl:col-span-2">
          <AdminCard className="h-full flex flex-col">
            ...
            <div className="flex-1 w-full rounded-2xl bg-gradient-to-b from-[var(--color-surface-2)]/80 to-[var(--color-surface-2)]/30 border border-[var(--color-border)] min-h-[350px] flex items-center justify-center mt-2 group relative overflow-hidden">
              ...
              <div className="text-center flex flex-col items-center opacity-40 group-hover:opacity-100 transition-opacity">
                ...
                <span className="text-sm font-medium tracking-widest uppercase text-[var(--color-text)]">Espacio para Gráfico Interactivo</span>
              </div>
            </div>
          </AdminCard>
        </div>
          ...
          <AdminCard className="h-full flex flex-col">
            ...
            <div className="flex-1 flex flex-col -mx-2 mt-2">
              {[
                ...
                { title: 'Reserva Confirmada', desc: 'Tour Eje Cafetero (2 pax)', time: 'Hace 22 min', dot: 'bg-[var(--color-success)]' },
                ...
                <div key={i} className="group relative flex items-start gap-4 p-3 rounded-xl hover:bg-[var(--color-surface-2)]/80 transition-colors cursor-pointer border-b border-[var(--color-border)] last:border-0">
                  <div className={`w-2 h-2 rounded-full mt-2 flex-shrink-0 shadow-sm ${item.dot}`} />
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-semibold text-[var(--color-text)] group-hover:text-brand-blue transition-colors truncate">
                      {item.title}
                    </p>
                  ...
                  <div className="flex flex-col items-end justify-start gap-1 shrink-0">
                    <span className="text-[10px] font-mono text-[var(--color-text-muted)] opacity-70 flex items-center">
                      <Clock className="w-3 h-3 mr-1" />
                      {item.time}
                    </span>
                    <ArrowUpRight className="w-3.5 h-3.5 text-[var(--color-text-muted)] opacity-0 -translate-y-1 group-hover:opacity-100 group-hover:translate-y-0 transition-all duration-300" />
                  </div>
                </div>
              ))}
            </div>
            
            <button className="w-full mt-6 py-2.5 rounded-xl border border-[var(--color-border)] text-xs font-bold uppercase tracking-wider text-[var(--color-text)] hover:bg-[var(--color-surface-2)] transition-colors flex items-center justify-center gap-2">
