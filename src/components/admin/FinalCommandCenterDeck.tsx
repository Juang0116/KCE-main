...
import { ShieldCheck, TrendingUp, Users, ArrowRight, Layers, Activity } from 'lucide-react';
...
export default function FinalCommandCenterDeck({
  ...
  return (
    ...
    >
      <div
        className={clsx(
          'grid gap-0 lg:grid-cols-[0.98fr_1.02fr]',
          ...
          <div className="relative z-10 space-y-6">
            ...
            <div className="grid grid-cols-2 gap-3 pt-2 sm:grid-cols-4">
              {['Open Truth', 'Push Calm', 'Revenue', 'Recovery'].map((label) => (
                <div
                  key={label}
                  className="rounded-xl border border-white/5 bg-white/5 px-3 py-2 text-center text-[10px] font-bold uppercase tracking-widest text-white/80"
                >
                  {label}
                </div>
              ))}
