...
export default function LocaleToggle({ className }: { className?: string }) {
  ...
  return (
    ...
    >
      ...
      <Globe className="h-3.5 w-3.5 text-[color:var(--color-text-muted)] transition-colors group-hover:text-brand-blue" />
      ...
      <label className="flex items-center">
        ...
        <select
          ...
          onChange={onChange}
          className={clsx(
            'text-[color:var(--color-text)]/70 cursor-pointer appearance-none bg-transparent text-xs font-bold tracking-widest outline-none',
            'pr-1 uppercase transition-colors group-hover:text-brand-blue',
          )}
        ...
        >
          {LOCALES.map((l) => (
            <option
              key={l.code}
              value={l.code}
              className="bg-[color:var(--color-surface)] text-[color:var(--color-text)] dark:bg-brand-dark dark:text-white"
            >
              {l.label}
            </option>
          ))}
        ...
        <span className="text-[color:var(--color-text)]/30 pointer-events-none text-[8px] transition-colors group-hover:text-brand-blue/50">
          ▼
        </span>
      </label>
    </div>
