...
export function ChatMarkdown({ content, tone = 'default' }: { content: string; tone?: Tone }) {
    ...
    <article className={clsx('space-y-3 text-[13px] leading-6', s.text)}>
      <ReactMarkdown
        ...
        components={{
          ...
          h1: ({ children }) => (
            <h1 className={clsx('text-base font-bold tracking-tight', s.accent)}>{children}</h1>
          ),
          h2: ({ children }) => (
            <h2 className={clsx('text-sm font-bold tracking-tight', s.accent)}>{children}</h2>
          ),
          h3: ({ children }) => (
            <h3 className={clsx('text-sm font-bold tracking-tight', s.accent)}>{children}</h3>
          ),
          ...
          ul: ({ children }) => (
            <ul className="my-2 list-disc space-y-1 pl-5 marker:text-brand-blue/50">{children}</ul>
          ),
          ol: ({ children }) => (
            <ol className="my-2 list-decimal space-y-1 pl-5 marker:font-bold">{children}</ol>
          ),
            ...
            return (
              <code
                className={clsx(
                  ...
                  isInline ? s.codeBg : 'block overflow-x-auto bg-brand-dark p-3 text-white',
                )}
              >
                {children}
              </code>
