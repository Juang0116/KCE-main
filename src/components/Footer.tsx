...
export default function Footer({ locale, dict }: Props) {
  return (
    <footer className="border-t border-[color:var(--color-border)] bg-[color:var(--color-surface)] pt-16 pb-8">
      <div className="mx-auto max-w-7xl px-6">
        
        {/* Newsletter & Brand Block */}
        <div className="grid gap-12 lg:grid-cols-[1.5fr_1fr] border-b border-[color:var(--color-border)] pb-16">
          <div className="max-w-md">
            <h3 className="font-heading text-3xl md:text-4xl text-brand-blue mb-4">
              {t(dict, 'brand.name', 'Knowing Cultures Enterprise')}
            </h3>
            <p className="text-sm font-light leading-relaxed text-[color:var(--color-text)]/70 mb-8">
        ...
        {/* Bottom Bar */}
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-t border-[color:var(--color-border)] pt-8">
          <div className="text-xs text-[color:var(--color-text)]/50">
            © {new Date().getFullYear()} Knowing Cultures Enterprise SAS. {t(dict, 'footer.rights', 'Todos los derechos reservados.')}
          </div>
          <div className="flex items-center gap-6">
            <span className="text-[10px] font-bold uppercase tracking-widest text-[color:var(--color-text)]/30">Operado desde Colombia</span>
