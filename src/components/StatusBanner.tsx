...
function normalizeKindFromHealth(out: any): BannerState {
  ...
  if (out?.configured === false) {
    return {
      ...
      message:
        'Configuración de seguridad incompleta. Algunas funciones de usuario están desactivadas.',
      requestId,
    };
  }
  ...
  const checkHealth = React.useCallback(async () => {
    ...
    if (typeof navigator !== 'undefined' && !navigator.onLine) {
      setState({
        ...
        kind: 'offline',
        message:
          'Sin conexión a internet. Los cambios no se guardarán hasta que vuelvas a estar en línea.',
      });
      return;
    }
      ...
      <div className="mx-auto flex max-w-7xl items-center justify-between gap-4 px-6 py-2.5">
        <div className="flex items-center gap-3">
          ...
          <div className="text-sm">
            ...
            {state.requestId && (
              <span className="ml-2 font-mono text-[10px] uppercase tracking-tighter opacity-50">
                ID: {state.requestId}
              </span>
            )}
          </div>
