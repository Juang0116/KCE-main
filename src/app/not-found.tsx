...
> = {
  ...
  de: {
    ...
    cta1: 'Touren erkunden',
    cta2: 'Zur Startseite',
    links: [
      ['Support 24h', '/contact'],
      ['Meine Buchungen', '/account/bookings'],
      ['Über KCE', '/about'],
    ],
  },
  es: {
...
export default function NotFound() {
  ...
  useEffect(() => {
    const lang =
      ...
      document.cookie.match(/kce\.locale=([^;]+)/)?.[1] ||
      'es';
      ...
      <main className="w-full max-w-2xl">
        <div className="relative overflow-hidden rounded-[2.5rem] border border-[color:var(--color-border)] bg-[color:var(--color-surface)] p-8 shadow-soft md:p-14">
          ...
          <div className="relative z-10">
            <div className="inline-flex items-center gap-2 rounded-full border border-brand-blue/15 bg-brand-blue/5 px-3 py-1 text-[10px] font-bold uppercase tracking-[0.25em] text-brand-blue">
              Error 404
            </div>
            <h1 className="mt-6 font-heading text-4xl text-brand-blue md:text-5xl">
              {c.title}
              <br />
              <span className="font-light text-[color:var(--color-text-muted)]">{c.sub}</span>
            </h1>
            ...
            <div className="mt-8 flex flex-wrap gap-3">
              <Link
                href={`${base}/tours`}
                className="inline-flex items-center gap-2 rounded-full bg-brand-blue px-6 py-3 text-sm font-semibold text-white transition-all hover:-translate-y-0.5 hover:bg-brand-blue/90"
              >
                {c.cta1} →
              </Link>
              <Link
                href={base}
                className="inline-flex items-center gap-2 rounded-full border border-[color:var(--color-border)] px-6 py-3 text-sm font-semibold text-[color:var(--color-text)] transition-all hover:bg-[color:var(--color-surface-2)]"
              >
                {c.cta2}
              </Link>
            </div>
