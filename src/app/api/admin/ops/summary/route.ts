...
export async function GET(req: NextRequest) {
  ...
  const [
    ...
    admin
      .from('bookings')
      .select('id, tour_title, customer_name, tour_date, status')
      .gte('tour_date', today)
      .eq('status', 'confirmed')
      .order('tour_date')
      .limit(5),
    ...
    admin
      ...
      .select('id, sequence_id, status, current_step, next_run_at')
      .eq('status', 'active')
      .order('next_run_at')
      .limit(10),
    admin
      .from('crm_outbound_messages')
      .select('id, status, to_email, subject, created_at, metadata')
  ...
  const todayBookings = (recentBookings ?? []).filter((b: any) => b.tour_date === today).length;

  const agentStats = {
    ops: {
      today: (agentEvents ?? []).filter(
        (e: any) => e.source === 'ops_agent' && e.type === 'ops_agent.completed',
      ).length,
      emails: (recentMessages ?? []).filter((m: any) => m.metadata?.agent === 'ops_agent').length,
    },
    review: {
      today: (agentEvents ?? []).filter(
        (e: any) => e.source === 'review_agent' && e.type === 'review_agent.completed',
      ).length,
      emails: (recentMessages ?? []).filter((m: any) => m.metadata?.agent === 'review_agent')
        .length,
    },
  };
