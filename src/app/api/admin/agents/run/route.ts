...
export async function POST(req: NextRequest) {
  ...
  if (dryRun)
    return NextResponse.json(
      { ok: true, requestId, results: { message: `Would run: ${agent}`, dryRun: true } },
  ...
  if (agent === 'analytics' || agent === 'all')
    await run('analytics', () => runAnalyticsAgent(requestId));
  if (agent === 'content' || agent === 'all')
    await run('content', () => runContentAgent(requestId));
  if (agent === 'trainer' || agent === 'all')
    await run('trainer', () => runTrainerAgent(requestId));
