...
function getProposalText(args: {
  ...
  const labels = {
    es: {
      intro: 'Te comparto la propuesta:',
      tour: 'Tour',
      date: 'Fecha',
      pax: 'Personas',
      total: 'Total',
      next: 'Para confirmar, paga aquí:',
    },
    en: {
      intro: 'Here is your proposal:',
      tour: 'Tour',
      date: 'Date',
      pax: 'Guests',
      total: 'Total',
      next: 'To confirm, pay here:',
    },
    de: {
      intro: 'Hier ist Ihr Angebot:',
      tour: 'Tour',
      date: 'Datum',
      pax: 'Personen',
      total: 'Gesamt',
      next: 'Zur Bestätigung hier bezahlen:',
    },
  }[lang];
  ...
  const lines = [
    ...
    checkoutUrl
      ? `${labels.next}\n${checkoutUrl}`
      : 'Si te parece bien, dime y te envío el link de pago.',
    '',
...
export async function POST(req: NextRequest, ctx: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    if (!tour)
      return NextResponse.json({ error: 'Tour no encontrado', requestId }, { status: 404 });
    ...
    const lang = parsed.data.locale?.startsWith('de')
      ? 'de'
      : parsed.data.locale?.startsWith('en')
        ? 'en'
        : 'es';
    ...
    void logEvent(
      ...
      { dealId, tour: tour.slug, stripeSessionId },
    ...
    return NextResponse.json(
      { ok: true, text, checkoutUrl, requestId },
      { status: 200, headers: withRequestId(undefined, requestId) },
    );
  } catch (err: any) {
    void logEvent(
      'api.error',
      { route: 'admin.proposal.create', error: err.message, requestId },
      { userId: auth.actor ?? null },
    );
