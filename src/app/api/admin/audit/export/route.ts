...
export async function POST(req: NextRequest) {
  ...
  if (!parsed.success)
    return NextResponse.json({ ok: false, error: 'Invalid body', requestId }, { status: 400 });
  ...
  if (agent === 'review' || agent === 'all')
    tasks.push(run('review', () => runReviewAgent(requestId)));
  if (agent === 'sales' || agent === 'all')
    tasks.push(run('sales', () => runSalesAgent(requestId)));
  if (agent === 'analytics' || agent === 'all')
    tasks.push(run('analytics', () => runAnalyticsAgent(requestId)));
  if (agent === 'content' || agent === 'all')
    tasks.push(run('content', () => runContentAgent(requestId)));
  if (agent === 'trainer' || agent === 'all')
    tasks.push(run('trainer', () => runTrainerAgent(requestId)));
