...
export async function GET(req: NextRequest) {
  ...
  if (!hasValidToken && !isVercelCron && hmacErr) {
    await logEvent(
      'security.unauthorized',
      {
        requestId,
        route: 'digest.cron',
        reason: 'Missing valid CRON_SECRET, Vercel header, or HMAC',
      },
      { source: 'api' },
    );
  ...
  try {
    ...
    await sendOpsDigestEmail({
      to,
      subject: `KCE Ops Digest - ${(revenueMinor / 100).toFixed(2)} ${currency}`,
      html,
    });
    ...
    await logEvent(
      'ops.digest_sent',
      {
        ...
        mitigationsCount: mitigations.length,
        recipients: to,
      },
    ...
    const errorMessage =
      error instanceof Error ? error.message : 'Error desconocido al enviar el digest';
