  ...
  withRequestId(req, async () => {
    ...
    try {
      ...
      if (body.scope === 'tour' && !body.tour_id) {
        return NextResponse.json(
          { error: 'tour_id es obligatorio para el scope "tour"', requestId },
          { status: 400 },
        );
      }
      if (body.scope === 'tag' && !body.tag) {
        return NextResponse.json(
          { error: 'tag es obligatorio para el scope "tag"', requestId },
          { status: 400 },
        );
      }
      if (body.scope === 'city' && !body.city) {
        return NextResponse.json(
          { error: 'city es obligatorio para el scope "city"', requestId },
          { status: 400 },
        );
      }
      ...
      return NextResponse.json(
        { error: err.message || 'Error interno', requestId },
        { status: 500 },
      );
    }
  });
