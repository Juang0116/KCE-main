...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (mode === 'required') {
      const token =
        req.headers.get('x-kce-action-token') || req.headers.get('x-admin-action-token');
      if (!token) {
        return NextResponse.json(
          { ok: false, error: 'Token de acción firmado requerido', requestId },
          { status: 403 },
        );
      }
      ...
      if (!verified.ok) {
        return NextResponse.json(
          {
            ok: false,
            error: 'Token de acción inválido o expirado',
            code: verified.code,
            requestId,
          },
          { status: 403 },
