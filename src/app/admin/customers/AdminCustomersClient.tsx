...
function readSaved(): SavedCustomerFilter[] {
  ...
  try {
    ...
    return parsed.map((x) => (typeof x === 'object' && x ? (x as SavedCustomerFilter) : null)).filter(Boolean) as SavedCustomerFilter[];
  } catch {
    return [];
  }
}
...
function writeSaved(filters: SavedCustomerFilter[]) {
  if (typeof window === 'undefined') return;
  try { window.localStorage.setItem(LS_KEY, JSON.stringify(filters.slice(0, 25))); } catch {}
}
...
export function AdminCustomersClient() {
  ...
  async function load() {
    ...
    try {
      ...
      setItems(json?.items || []); setTotal(json?.total ?? null);
  ...
  async function saveAsSegment() {
    ...
    const qv = q.trim(); const countryv = country.trim(); const languagev = language.trim();

    try {
      const payload = {
        name, entity_type: 'customers',
        filter: { ...(qv ? { q: qv } : {}), ...(countryv ? { country: countryv } : {}), ...(languagev ? { language: languagev } : {}) },
      };
  ...
  function applySegment(id: string) {
    const seg = segments.find((x) => x.id === id); if (!seg) return;
    const f = seg.filter || {};
    setQ(typeof (f as any).q === 'string' ? (f as any).q : '');
    setCountry(typeof (f as any).country === 'string' ? (f as any).country : '');
    setLanguage(typeof (f as any).language === 'string' ? (f as any).language : '');
    setPage(1);
  }

  useEffect(() => { void load(); }, [q, country, language, page]);
  useEffect(() => { void loadSegments(); }, []);
  useEffect(() => { setSaved(readSaved()); }, []);

  function applySaved(name: string) {
    const f = saved.find((x) => x.name === name); if (!f) return;
    setQ(f.q || ''); setCountry(f.country || ''); setLanguage(f.language || ''); setPage(1);
  }

  function saveCurrent() {
    const name = saveName.trim(); if (!name) return;
    const next: SavedCustomerFilter = { name, ...(q.trim() ? { q: q.trim() } : {}), ...(country.trim() ? { country: country.trim() } : {}), ...(language.trim() ? { language: language.trim() } : {}) };
  ...
  function exportCsv() {
    const qs = new URLSearchParams();
    if (q.trim()) qs.set('q', q.trim()); if (country.trim()) qs.set('country', country.trim()); if (language.trim()) qs.set('language', language.trim());
    window.open(`/api/admin/customers/export?${qs.toString()}`, '_blank', 'noopener,noreferrer');
  }

  const customerSignals = useMemo(() => [
    { label: 'Visible customers', value: total != null ? String(total) : String(items.length), note: 'Registros en esta vista.' },
    { label: 'With country', value: String(items.filter((item) => Boolean(item.country)).length), note: 'Clientes con país definido.' },
    { label: 'With language', value: String(items.filter((item) => Boolean(item.language)).length), note: 'Clientes con idioma detectado.' },
    { label: 'DB segments', value: String(segments.length), note: 'Segmentos globales guardados.' },
      ...
      <div className="rounded-[2.5rem] border border-[var(--color-border)] bg-[var(--color-surface)] p-6 md:p-8 shadow-sm">
        ...
        <div className="mb-8 flex flex-wrap items-center gap-4 rounded-2xl border border-brand-blue/15 bg-brand-blue/5 p-4 md:p-5">
          ...
          <div className="flex items-center gap-2">
            <select value={selectedSaved} onChange={(e) => { const v = e.target.value; setSelectedSaved(v); if (v) applySaved(v); }} className="h-10 rounded-xl border border-brand-blue/20 bg-white/60 px-3 text-sm outline-none w-40 text-brand-blue/70">
          ...
          <div className="flex items-center gap-2">
            <select value={selectedSegment} onChange={(e) => { const v = e.target.value; setSelectedSegment(v); if (v) applySegment(v); }} className="h-10 rounded-xl border border-brand-blue/20 bg-white/60 px-3 text-sm outline-none w-40 text-brand-blue/70">
        ...
        <div className="overflow-x-auto rounded-3xl border border-[var(--color-border)] bg-white shadow-sm">
          <table className="w-full text-left text-sm min-w-[1000px]">
            ...
            <tbody className="divide-y divide-[var(--color-border)]">
              {loading ? (
                <tr><td colSpan={5} className="px-6 py-16 text-center text-[var(--color-text)]/40 font-medium">Buscando en la base de datos...</td></tr>
              ) : items.length > 0 ? (
                items.map((c) => (
                  <tr key={c.id} className="transition-colors hover:bg-[var(--color-surface-2)]/50">
                    <td className="px-6 py-5 align-top">
                      ...
                      <div className="mt-1 text-[10px] font-mono text-[var(--color-text)]/30 uppercase tracking-widest">ID: {c.id.slice(0,8)}</div>
                    </td>
                    <td className="px-6 py-5 align-top">
                      <div className="font-medium text-[var(--color-text)]">{c.email || '—'}</div>
                      <div className="mt-1 text-xs text-[var(--color-text)]/60 font-mono">{c.phone || 'Sin Teléfono'}</div>
                    </td>
                    <td className="px-6 py-5 align-top text-center">
                      <div className="flex justify-center gap-2">
                        <span className="inline-flex items-center rounded-lg bg-[var(--color-surface-2)] px-3 py-1 text-[10px] font-bold uppercase tracking-widest text-[var(--color-text)] border border-[var(--color-border)]">
                          <Globe className="h-3 w-3 mr-1.5 opacity-50"/> {c.country || 'N/A'}
                        </span>
                        <span className="inline-flex items-center rounded-lg bg-[var(--color-surface-2)] px-3 py-1 text-[10px] font-bold uppercase tracking-widest text-[var(--color-text)] border border-[var(--color-border)]">
                          <Languages className="h-3 w-3 mr-1.5 opacity-50"/> {c.language || 'N/A'}
                        </span>
                      </div>
                    </td>
                    <td className="px-6 py-5 align-top text-[10px] font-bold uppercase tracking-widest text-[var(--color-text)]/50">
                      {new Date(c.created_at).toLocaleDateString('es-ES')}
                    </td>
                    <td className="px-6 py-5 align-top text-right">
                      <Link href={`/admin/customers/${encodeURIComponent(c.id)}`} className="inline-flex items-center justify-center rounded-xl bg-brand-dark px-5 py-2.5 text-[10px] font-bold uppercase tracking-widest text-brand-yellow transition hover:scale-105 shadow-sm">
                        Abrir CRM 360
                      </Link>
                    </td>
                  </tr>
                ...
                <tr>
                  <td colSpan={5} className="px-6 py-16 text-center">
                    ...
                    <div className="text-sm font-medium text-[var(--color-text)]/40">No hay clientes que coincidan con estos filtros.</div>
                  </td>
                </tr>
              )}
            </tbody>
        ...
        {pages && pages > 1 && (
          <div className="mt-6 flex items-center justify-between border-t border-[var(--color-border)] pt-6">
            <button disabled={page <= 1 || loading} onClick={() => setPage((p) => Math.max(1, p - 1))} className="rounded-xl border border-[var(--color-border)] bg-[var(--color-surface-2)] px-5 py-2.5 text-[10px] font-bold uppercase tracking-widest disabled:opacity-30 transition hover:bg-[var(--color-surface)]">
              ← Anterior
            </button>
            <div className="text-[10px] font-bold uppercase tracking-widest text-[var(--color-text)]/50">
              Página {page} de {pages}
            </div>
            <button disabled={page >= pages || loading} onClick={() => setPage((p) => p + 1)} className="rounded-xl border border-[var(--color-border)] bg-[var(--color-surface-2)] px-5 py-2.5 text-[10px] font-bold uppercase tracking-widest disabled:opacity-30 transition hover:bg-[var(--color-surface)]">
              Siguiente →
            </button>
          </div>
        )}
