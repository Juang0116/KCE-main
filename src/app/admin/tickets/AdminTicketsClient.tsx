...
function badge(kind: 'status' | 'priority', value: string) {
  ...
  if (kind === 'status') {
    ...
    if (v === 'in_progress')
      return `${base} border border-brand-blue/20 bg-brand-blue/10 text-brand-blue`;
    if (v === 'resolved')
      return `${base} border border-emerald-500/20 bg-emerald-500/10 text-emerald-700`;
  }
  if (kind === 'priority') {
...
function slaBadgeFor(ticket: Ticket) {
  ...
  if (h == null)
    return {
      label: '—',
      className: 'text-[color:var(--color-text)]/50 bg-[color:var(--color-surface-2)]',
    };
  if (h >= SLA_BREACH_HOURS)
    return { label: 'BREACH', className: 'text-white bg-rose-600 animate-pulse' };
  if (h >= SLA_WARN_HOURS) return { label: 'AT RISK', className: 'text-amber-800 bg-amber-200' };
  ...
  const ticketSignals = useMemo(
    () => [
      ...
      },
      {
        label: 'Active now',
        value: String(
          items.filter((ticket) => (ticket.status || '').toLowerCase() !== 'resolved').length,
        ),
      ...
      },
      {
        ...
        value: String(
          items.filter((ticket) => (ticket.priority || '').toLowerCase() === 'urgent').length,
        ),
  ...
  return (
    <div className="space-y-10 pb-20">
      ...
      <div className="flex flex-col justify-between gap-6 md:flex-row md:items-end">
        <div>
          <h1 className="font-heading text-3xl text-brand-blue md:text-4xl">Support Desk</h1>
      ...
      {/* 2. FILTROS */}
      <div className="grid gap-4 rounded-3xl border border-[color:var(--color-border)] bg-[color:var(--color-surface)] p-6 shadow-sm sm:grid-cols-2 md:grid-cols-4">
        <div>
          ...
          <select
            ...
            className="w-full appearance-none rounded-xl border border-[color:var(--color-border)] bg-[color:var(--color-surface-2)] px-3 py-2.5 text-sm outline-none focus:border-brand-blue"
          >
        ...
        <div className="md:col-span-2">
          ...
          <div className="relative">
            <Search className="text-[color:var(--color-text)]/30 absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2" />
            <input
              value={q}
              onChange={(e) => {
                setQ(e.target.value);
              ...
              placeholder="Asunto, resumen, ID..."
              className="w-full rounded-xl border border-[color:var(--color-border)] bg-[color:var(--color-surface-2)] px-3 py-2.5 pl-10 text-sm outline-none focus:border-brand-blue"
            />
          </div>
        </div>
      </div>
      ...
      <div className="overflow-x-auto rounded-3xl border border-[color:var(--color-border)] bg-[color:var(--color-surface)] shadow-sm">
        <div className="flex items-center justify-between border-b border-[color:var(--color-border)] bg-[color:var(--color-surface-2)] px-6 py-4">
          <div className="text-[color:var(--color-text)]/50 text-[10px] font-bold uppercase tracking-widest">
        ...
        {loading ? (
          ...
          <div className="p-20 text-center">
            <ShieldCheck className="mx-auto mb-4 h-12 w-12 text-emerald-500/30" />
            <h3 className="font-heading text-xl text-[color:var(--color-text)]">Todo en orden</h3>
            <p className="text-[color:var(--color-text)]/50 mt-1 text-sm">
              No hay tickets pendientes en esta vista.
            </p>
          </div>
        ) : (
          <div className="divide-y divide-[var(--color-border)]">
            {items.map((t) => (
              <Link
                ...
                className="hover:bg-[color:var(--color-surface-2)]/60 block bg-[color:var(--color-surface)] p-6 transition-colors"
              >
                <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                  <div className="min-w-0 flex-1">
                    <div className="mb-2 flex flex-wrap items-center gap-2">
                      ...
                      <span className="text-[color:var(--color-text)]/30 font-mono text-[10px]">
                        #{t.id.slice(0, 8)}
                      </span>
                    </div>
                    <div className="text-base font-semibold leading-tight text-[color:var(--color-text)]">
                      {t.subject || 'Sin Asunto'}
                    </div>
                    <div className="text-[color:var(--color-text)]/60 mt-1 line-clamp-1 text-sm font-light">
                      {t.summary || 'Sin resumen...'}
                    </div>
                  </div>

                  <div className="flex shrink-0 flex-col items-start text-right sm:items-end">
                    <div
                      className={`inline-flex items-center gap-1.5 rounded-full px-3 py-1 text-[10px] font-bold uppercase tracking-widest ${slaBadgeFor(t).className}`}
                    >
                      <Clock className="h-3 w-3" /> SLA: {slaBadgeFor(t).label}
                    </div>
                    <div className="text-[color:var(--color-text)]/50 mt-2 text-xs">
                      Creado: {fmtDate(t.created_at)}
                    </div>
                    <div className="mt-1 text-xs font-semibold text-[color:var(--color-text-muted)]">
                      Abierto hace {fmtAge(ageHours(t.created_at))}
                    </div>
                  </div>
                </div>
              </Link>
        ...
        {total != null && total > limit && (
          <div className="bg-[color:var(--color-surface-2)]/50 flex items-center justify-between border-t border-[color:var(--color-border)] px-6 py-4">
            <button
              onClick={() => setPage((p) => Math.max(1, p - 1))}
              disabled={!hasPrev || loading}
              className="rounded-xl border border-[color:var(--color-border)] bg-[color:var(--color-surface)] px-4 py-2 text-xs font-bold uppercase tracking-widest disabled:opacity-50"
            >
              ← Anterior
            </button>
            <div className="text-[color:var(--color-text)]/50 text-[10px] font-bold uppercase tracking-widest">
