    ...
    <main className="space-y-12 pb-24 animate-in fade-in slide-in-from-bottom-4 duration-700">
      ...
      <header className="flex flex-col md:flex-row md:items-end justify-between gap-8 border-b border-brand-dark/5 dark:border-white/5 pb-10">
        <div className="space-y-4">
          <Link 
            href="/admin/content" 
            className="group inline-flex items-center gap-2 text-[10px] font-bold uppercase tracking-[0.2em] text-muted hover:text-red-600 transition-colors"
          >
            <ArrowLeft className="h-3.5 w-3.5 transition-transform group-hover:-translate-x-1" /> Production Suite / Hub
          </Link>
          <div>
            ...
            <h1 className="font-heading text-4xl md:text-5xl text-main tracking-tighter">
              Catálogo de <span className="text-red-600 italic font-light">Vlog</span>
            </h1>
          </div>
        </div>
      ...
      <section className="grid gap-6 sm:grid-cols-3">
        {[
          { label: 'Activos Visuales', value: totalVideos, color: 'text-red-600', icon: MonitorPlay },
          { label: 'Streaming Activo', value: publishedCount, color: 'text-green-600 dark:text-green-400', icon: Globe },
      ...
      {error && (
        <div className="rounded-[var(--radius-2xl)] border border-red-500/20 bg-red-50 dark:bg-red-950/20 p-6 flex items-center gap-4 text-red-700 dark:text-red-400 animate-in slide-in-from-top-2 shadow-sm">
          <AlertTriangle className="h-6 w-6 shrink-0" />
          <p className="text-sm font-bold">Error de Sincronización Multimedia: <span className="font-light">{error.message}</span></p>
        </div>
      )}
      ...
      <section className="rounded-[var(--radius-3xl)] border border-brand-dark/5 dark:border-white/5 bg-surface shadow-pop overflow-hidden flex flex-col relative">
        <div className="p-8 pb-6 flex items-center justify-between border-b border-brand-dark/5 dark:border-white/5 bg-surface-2/30">
           <div className="flex items-center gap-3">
             ...
             <h2 className="font-heading text-2xl text-main tracking-tight">Directorio de Producciones</h2>
           </div>
              ...
              {items.length === 0 ? (
                <tr>
                  <td colSpan={5} className="px-8 py-32 text-center bg-surface">
                    <Video className="mx-auto h-16 w-16 text-red-600 opacity-10 mb-6" />
                    <p className="text-xl font-heading text-main tracking-tight opacity-30">Estudio en Silencio</p>
                    <p className="text-sm font-light text-muted mt-2 italic">No hay material visual registrado. Inicia una nueva producción.</p>
                  </td>
                </tr>
              ) : (
                items.map((v) => {
                  ...
                  return (
                    <tr key={v.id} className="group transition-colors hover:bg-surface-2/50 cursor-default bg-surface">
                      <td className="px-8 py-6 align-top">
                        <div className="font-heading text-xl text-main group-hover:text-red-600 transition-colors leading-tight tracking-tight line-clamp-1">
                          {v.title || <span className="opacity-20 italic">Corte sin título</span>}
                        </div>
                        <div className="mt-2 flex flex-col gap-2">
                          {v.youtube_url && (
                            <a 
                              ...
                              target="_blank" 
                              rel="noreferrer" 
                              className="inline-flex items-center gap-2 text-[10px] font-mono text-brand-blue/60 hover:text-brand-blue transition-colors w-fit bg-brand-blue/5 px-2 py-0.5 rounded border border-brand-blue/5"
                            >
                              <Youtube className="h-3 w-3" /> {v.youtube_url.substring(0, 45)}...
                              <ExternalLink className="h-2.5 w-2.5 opacity-0 group-hover:opacity-100 transition-opacity" />
                            </a>
                          )}
                          <div className="text-[9px] font-mono text-muted uppercase tracking-widest opacity-40">
                      ...
                      <td className="px-8 py-6 align-top text-center">
                        <div className="inline-flex items-center gap-2 rounded-xl bg-surface-2 border border-brand-dark/10 dark:border-white/10 px-4 py-2 shadow-inner">
                          ...
                          <span className="text-[10px] font-bold uppercase text-main">{(v.lang ?? 'ES')}</span>
                        </div>
                      </td>
                      ...
                      <td className="px-8 py-6 align-top text-right">
                        <div className="flex flex-col items-end">
                          <div className="text-xs font-bold text-main flex items-center gap-2">
                            <Clock className="h-3.5 w-3.5 opacity-30 text-red-600" />
                            {v.updated_at ? new Date(v.updated_at).toLocaleDateString('es-CO', { day: '2-digit', month: 'short', year: 'numeric' }) : '—'}
                          </div>
                          <div className="mt-1 text-[9px] font-bold uppercase tracking-[0.2em] text-muted opacity-40">Edición final</div>
                        </div>
                      </td>

                      <td className="px-8 py-6 align-top text-right">
                        <Button variant="ghost" size="sm" asChild className="h-10 rounded-xl hover:bg-red-600/5 text-red-600 group/btn px-4 transition-all">
                          <Link href={`/admin/content/videos/${v.id}`}>
                            <Edit3 className="mr-2 h-4 w-4" /> 
                            <span className="text-[10px] font-bold uppercase tracking-widest">Editar Material</span>
        ...
        <footer className="mt-auto flex flex-col sm:flex-row items-center justify-between border-t border-brand-dark/5 dark:border-white/5 bg-surface-2/30 p-8 opacity-60">
          ...
          <div className="flex items-center gap-8 text-[10px] font-mono tracking-widest uppercase text-muted mt-4 sm:mt-0">
             <div className="flex items-center gap-2">
                <ShieldCheck className="h-3.5 w-3.5 opacity-40 text-green-500" /> YouTube Sync: Verified
             </div>
             <div className="flex items-center gap-2">
                <Zap className="h-3.5 w-3.5 opacity-40 text-brand-yellow" /> RAG-Extraction: Active
             </div>
          </div>
        </footer>
      </section>
