...
export const metadata: Metadata = {
  ...
  description:
    'Gestión de filtros inmutables y segmentación de base de datos para Knowing Cultures Enterprise.',
...
export default async function AdminSegmentsPage() {
  ...
  return (
    <main className="animate-in fade-in slide-in-from-bottom-2 mx-auto max-w-[1500px] space-y-12 p-6 pb-24 duration-700">
      ...
      <header className="flex flex-col justify-between gap-8 border-b border-[color:var(--color-border)] px-2 pb-10 md:flex-row md:items-center">
        <div className="space-y-3">
          ...
          <h1 className="font-heading text-4xl leading-tight tracking-tight text-brand-blue md:text-5xl">
            Matriz de <span className="font-light italic text-brand-yellow">Segmentos</span>
          </h1>
          <p className="text-[color:var(--color-text)]/50 max-w-2xl text-base font-light italic leading-relaxed">
            Unidad de definición de cohortes. Guarda filtros complejos y reutilizables en el núcleo
            de la DB para ejecutar conteos de rastro y disparar acciones de marketing con precisión
            quirúrgica.
          </p>
        </div>

        {/* Status de Integridad de Datos */}
        <div className="group flex items-center gap-6 rounded-[2.5rem] border border-brand-blue/10 bg-brand-blue/5 p-6 shadow-inner">
          ...
          <div>
            ...
            <p className="font-mono text-xs font-bold uppercase tracking-tighter text-emerald-600">
              Query Node Active
            </p>
          </div>
        </div>
      ...
      <div className="flex items-center justify-between px-2">
        <Link
          href="/admin"
          className="group flex items-center gap-2 text-[10px] font-bold uppercase tracking-widest text-[color:var(--color-text-muted)] transition-colors hover:text-brand-blue"
        >
        ...
        <div className="flex items-center gap-8">
          <div className="text-[color:var(--color-text)]/30 flex items-center gap-2 text-[10px] font-bold uppercase tracking-widest">
      ...
      <footer className="mt-20 flex items-center justify-center gap-12 border-t border-[color:var(--color-border)] pt-12 opacity-20 transition-opacity duration-500 hover:opacity-50">
