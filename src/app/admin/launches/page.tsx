    ...
    <main className="mx-auto max-w-7xl space-y-12 p-6 pb-32 animate-in fade-in slide-in-from-bottom-2 duration-700">
      ...
      <header className="flex flex-col md:flex-row md:items-end justify-between gap-8 border-b border-[var(--color-border)] pb-10">
        <div className="space-y-4">
          ...
          <p className="text-lg text-[var(--color-text)]/50 font-light max-w-2xl italic leading-relaxed">
                ...
                launches.map((x) => (
                  <tr key={x.id} className="group transition-all hover:bg-brand-blue/[0.01]">
                    <td className="px-8 py-6 align-top">
                      ...
                      <div className="mt-1 text-[9px] font-mono text-[var(--color-text)]/30">TRACE_ID: {x.id.slice(0, 8)}</div>
                    </td>
                    <td className="px-8 py-6 align-top">
                      <div className="flex items-center gap-2 text-brand-dark font-medium uppercase tracking-tighter">
                    ...
                    <td className="px-8 py-6 align-top text-center">
                      <span className={`inline-flex items-center rounded-lg border px-3 py-1 text-[9px] font-bold uppercase tracking-widest shadow-inner ${
                        x.status === 'active' ? 'border-emerald-500/20 bg-emerald-500/5 text-emerald-600' : 'border-[var(--color-border)] bg-[var(--color-surface-2)] text-[var(--color-text)]/40'
                      }`}>
                        {x.status ?? 'Pending'}
                      </span>
                    </td>
                    ...
                    <td className="px-8 py-6 align-top text-right max-w-[400px]">
                      <p className="text-xs font-light text-[var(--color-text)]/60 line-clamp-2 italic" title={x.notes ?? ''}>
      ...
      <section className="grid gap-8 lg:grid-cols-[1fr_400px]">
              ...
              {[
                ...
                <div key={i} className="flex gap-4 group">
                  ...
                  <div className="space-y-1">
                    <h4 className="font-bold text-sm uppercase tracking-widest text-brand-yellow/80">{step.t}</h4>
        ...
        <div className="rounded-[3rem] border border-[var(--color-border)] bg-[var(--color-surface)] p-8 space-y-6 shadow-sm flex flex-col justify-center">
           <div className="flex items-center gap-3 text-brand-blue">
              <ShieldCheck className="h-6 w-6" />
              <h4 className="font-heading text-xl">Expansion Guard</h4>
           </div>
           <p className="text-sm font-light text-[var(--color-text)]/60 leading-relaxed italic border-l-2 border-brand-blue/20 pl-4">
           ...
           <div className="pt-4 border-t border-[var(--color-border)] flex items-center justify-between text-[10px] font-bold uppercase tracking-widest text-[var(--color-text)]/30">
