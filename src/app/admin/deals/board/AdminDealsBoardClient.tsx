...
export function AdminDealsBoardClient() {
  ...
  return (
    <div className="space-y-12 pb-24 animate-in fade-in slide-in-from-bottom-2 duration-700">
      ...
      <div className="rounded-[3rem] border border-[color:var(--color-border)] bg-[color:var(--color-surface)] p-2 shadow-2xl relative overflow-hidden">
        
        {/* Barra de Filtros */}
        <div className="p-6 md:p-8 flex flex-col lg:flex-row items-center justify-between gap-6 mb-4 border-b border-[color:var(--color-border)] bg-white/50 backdrop-blur-sm rounded-t-[2.5rem]">
          <div className="relative w-full lg:w-1/2 group">
            ...
            <input 
              ...
              className="w-full h-14 pl-14 rounded-2xl border border-[color:var(--color-border)] bg-[color:var(--color-surface)] text-sm font-light outline-none focus:ring-4 focus:ring-brand-blue/5 transition-all" 
            />
          </div>
          <Button variant="outline" onClick={() => void load()} disabled={loading} className="rounded-full px-10 py-7 shadow-sm bg-[color:var(--color-surface)]">
          ...
          {STAGES.map((stage) => {
            ...
            return (
              <div key={stage} className={`flex-shrink-0 w-[380px] snap-center rounded-[2.5rem] border p-5 flex flex-col transition-colors ${
                isHot ? 'bg-brand-blue/[0.03] border-brand-blue/10 shadow-inner' : 'bg-[color:var(--color-surface-2)] border-[color:var(--color-border)]'
              }`}>
                ...
                <header className="flex items-center justify-between mb-6 px-4">
                  ...
                  <div className="rounded-xl bg-[color:var(--color-surface)] border border-[color:var(--color-border)] px-3 py-1.5 text-[10px] font-bold text-brand-blue/40 shadow-sm">
                    {columnDeals.length}
                  </div>
                </header>
                ...
                <div className="flex-1 space-y-5 overflow-y-auto pr-2 custom-scrollbar">
                  {columnDeals.map((d) => {
                    ...
                    return (
                      <div key={d.id} className="rounded-[2rem] border border-[color:var(--color-border)] bg-[color:var(--color-surface)] p-6 shadow-sm transition-all hover:shadow-2xl hover:border-brand-blue/20 group">
                        
                        <header className="flex items-start justify-between gap-4 mb-4">
                          <Link href={`/admin/deals/${d.id}`} className="font-heading text-lg text-brand-blue group-hover:text-brand-blue transition-colors leading-tight line-clamp-2">
                            {d.title || 'Manuscrito de Venta'}
                          </Link>
                          <span className="font-mono text-[9px] text-[color:var(--color-text)]/50 uppercase tracking-tighter shrink-0">#{d.id.slice(0,6)}</span>
                        </header>
                        ...
                        <div className="space-y-3 pt-5 border-t border-[color:var(--color-border)]">
                          <select 
                            className="w-full h-11 rounded-xl border border-[color:var(--color-border)] bg-[color:var(--color-surface-2)] px-4 text-[9px] font-bold uppercase tracking-[0.2em] text-brand-blue/60 outline-none focus:border-brand-blue appearance-none cursor-pointer text-center" 
                        ...
                        {isOpen && (
                          <div className="mt-5 space-y-4 p-5 rounded-3xl bg-[color:var(--color-surface-2)] border border-[color:var(--color-border)] animate-in zoom-in-95 duration-300">
                             <AdminTourSelector value={draft.slug} onChange={(slug) => patchDraft(d.id, { slug }, draft)} />
                             <div className="grid grid-cols-2 gap-2">
                                <input type="date" value={draft.date} onChange={(e) => patchDraft(d.id, { date: e.target.value }, draft)} className="w-full h-11 rounded-xl border border-[color:var(--color-border)] bg-[color:var(--color-surface)] px-3 text-xs outline-none" />
                                <div className="flex items-center bg-[color:var(--color-surface)] border border-[color:var(--color-border)] rounded-xl overflow-hidden h-11">
