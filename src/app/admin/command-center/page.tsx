      ...
      <div className="mx-auto max-w-7xl space-y-12 p-6 pb-32 animate-in fade-in slide-in-from-bottom-2 duration-700">
        ...
        <header className="flex flex-col md:flex-row md:items-center justify-between gap-8 border-b border-[color:var(--color-border)] pb-10">
          <div>
            ...
            <h1 className="font-heading text-4xl md:text-6xl text-brand-blue">
               Command <span className="text-brand-blue italic font-light text-5xl md:text-7xl">Center</span>
            </h1>
            <p className="mt-4 text-base text-[color:var(--color-text)]/50 font-light italic flex items-center gap-2">
        ...
        <section className="relative overflow-hidden rounded-[3.5rem] border border-brand-blue/20 bg-brand-dark p-1 shadow-2xl transition-all hover:shadow-brand-blue/10">
          <div className="absolute top-0 right-0 p-12 opacity-[0.08] transition-transform hover:scale-110 duration-1000">
             <Bot className="h-72 w-72 text-brand-blue" />
          </div>
          <div className="relative z-10 bg-brand-dark px-10 py-12 md:px-16 md:py-20">
            <div className="mb-10 flex items-center gap-5">
               <div className="flex h-14 w-14 items-center justify-center rounded-2xl bg-brand-yellow text-[color:var(--color-text)] shadow-xl shadow-brand-yellow/20">
                  <Sparkles className="h-8 w-8" />
               </div>
               <div>
                  <h2 className="font-heading text-3xl text-white">Agente CEO</h2>
                  <p className="text-[11px] font-bold uppercase tracking-[0.4em] text-brand-blue/60">Briefing Matutino Estratégico</p>
               </div>
            </div>
            ...
            <div className="mt-16 flex items-center gap-6 border-t border-white/5 pt-10">
               <div className="flex items-center gap-2 text-[10px] font-bold uppercase tracking-[0.3em] text-white/40">
                  <Cpu className="h-3.5 w-3.5 text-brand-blue" /> Inferencia Neural Activa
               </div>
               <div className="flex items-center gap-2 text-[10px] font-bold uppercase tracking-[0.3em] text-white/40">
                  <Target className="h-3.5 w-3.5 text-brand-blue" /> Focus Mode: Revenue Recovery
               </div>
            </div>
          </div>
        </section>
        ...
        <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
          ...
          <StatCard title="Pipeline (EUR)" value={`€${stats.potentialRevenue.toLocaleString()}`} icon={TrendingUp} colorClass="text-brand-blue" />
          <StatCard 
        ...
        <section className="rounded-[3.5rem] border border-[color:var(--color-border)] bg-[color:var(--color-surface)] p-10 md:p-16 shadow-inner relative overflow-hidden">
          ...
          <div className="relative z-10">
            <div className="mb-12 flex items-center justify-between border-b border-[color:var(--color-border)] pb-8">
               ...
               <span className="text-[10px] font-bold uppercase tracking-[0.3em] text-[color:var(--color-text)]/30">Acceso Nivel 0</span>
            </div>
        ...
        <section className="space-y-6">
          <div className="flex items-center gap-4 px-6">
             ...
             <div className="text-[11px] font-bold uppercase tracking-[0.4em] text-[color:var(--color-text)]/30">Live System Heartbeat & Logistics</div>
          </div>
          <div className="rounded-[3rem] border border-[color:var(--color-border)] bg-white/50 p-2 shadow-sm backdrop-blur-sm">
        ...
        <footer className="pt-20 text-center opacity-30 transition-opacity hover:opacity-60 duration-500">
           ...
           <p className="text-[10px] font-bold uppercase tracking-[0.5em] text-[color:var(--color-text)] italic">
...
function StatCard({ title, value, icon: Icon, colorClass, alert }: StatCardProps) {
  return (
    <div className={`group relative overflow-hidden rounded-[2.5rem] border border-[color:var(--color-border)] bg-[color:var(--color-surface)] p-10 shadow-sm transition-all hover:shadow-2xl hover:-translate-y-1 ${alert ? 'border-rose-100 bg-rose-50/[0.05]' : ''}`}>
      <div className="flex items-center justify-between mb-8">
        <div className={`rounded-2xl p-4 bg-[color:var(--color-surface-2)] ${colorClass} transition-all group-hover:scale-110 shadow-inner`}>
      ...
      <div>
        <p className="text-[10px] font-bold uppercase tracking-[0.2em] text-[color:var(--color-text)]/30 mb-2">{title}</p>
        <p className={`text-4xl font-heading tracking-tight ${alert ? 'text-rose-600' : 'text-[color:var(--color-text)]'}`}>{value}</p>
      </div>
    </div>
  );
}
...
function ActionLink({ href, title, desc, icon: Icon }: ActionLinkProps) {
  return (
    <a href={href} className="group rounded-[2.5rem] border border-[color:var(--color-border)] bg-[color:var(--color-surface)] p-8 shadow-sm transition-all hover:shadow-2xl hover:-translate-y-2">
      <div className="flex items-start justify-between mb-8">
        ...
        <ArrowUpRight className="h-5 w-5 text-[color:var(--color-text)]/50 group-hover:text-brand-blue transition-colors" />
      </div>
      <div>
        <h4 className="font-heading text-xl text-brand-blue group-hover:text-brand-blue transition-colors">{title}</h4>
        <p className="mt-3 text-xs font-light text-[color:var(--color-text)]/50 leading-relaxed group-hover:text-[color:var(--color-text)]/80 transition-colors">{desc}</p>
      </div>
