    ...
    <PageShell className="mx-auto w-full max-w-[var(--container-max)] px-6 py-12 md:py-20 animate-fade-in">
      ...
      <header className="mb-12 flex flex-col md:flex-row md:items-end justify-between gap-6 border-b border-[color:var(--color-border)] pb-8">
        ...
        <div className="flex flex-wrap items-center gap-4 shrink-0">
          ...
          <Button asChild className="rounded-full bg-[color:var(--color-success)] text-white shadow-pop hover:-translate-y-0.5 transition-transform">
        ...
        <div className="space-y-12">
          ...
          <section className="rounded-[var(--radius-2xl)] border border-[color:var(--color-border)] bg-[color:var(--color-surface)] shadow-soft overflow-hidden">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-[color:var(--color-border)] bg-[color:var(--color-surface-2)]/30 p-6 md:p-8">
              <div>
                <h2 className="font-heading text-2xl text-[color:var(--color-text)] tracking-tight">Centro de Asistencia</h2>
                <p className="text-xs text-[color:var(--color-text-muted)] mt-1 font-light">Listado de tickets activos e históricos.</p>
              </div>
        ...
        <aside className="space-y-8">
          ...
          <div className="rounded-[var(--radius-2xl)] border border-[color:var(--color-border)] bg-[color:var(--color-surface)]/80 backdrop-blur-xl p-8 shadow-soft relative overflow-hidden group">
            ...
            <div className="relative z-10">
              <p className="text-[10px] font-bold uppercase tracking-[0.3em] text-[color:var(--color-text-muted)] mb-8 border-b border-[color:var(--color-border)] pb-4">Tips de Soporte</p>
              
              <div className="space-y-8">
                <div className="flex items-start gap-4 group/item">
                  <div className="rounded-xl border border-[color:var(--color-border)] bg-[color:var(--color-surface-2)] p-2.5 text-[color:var(--color-text-muted)] transition-colors group-hover/item:border-[color:var(--color-success)] group-hover/item:text-[color:var(--color-success)] shrink-0">
                    <ShieldCheck className="h-5 w-5" />
                  </div>
                  <div className="pt-0.5">
                    <p className="font-heading text-lg text-[color:var(--color-text)] mb-1 group-hover/item:text-[color:var(--color-success)] transition-colors">Booking ID</p>
                    <p className="text-sm font-light text-[color:var(--color-text-muted)] leading-relaxed">Selecciona la reserva correcta para acelerar la resolución.</p>
                  </div>
                </div>
                ...
                <div className="flex items-start gap-4 group/item">
                  ...
                  <div className="pt-0.5">
                    <p className="font-heading text-lg text-[color:var(--color-text)] mb-1 group-hover/item:text-brand-blue transition-colors">Sé Específico</p>
                    <p className="text-sm font-light text-[color:var(--color-text-muted)] leading-relaxed">Explica qué pasó, qué esperabas y qué necesitas del equipo.</p>
                  </div>
                </div>
                
                <div className="flex items-start gap-4 group/item">
                  <div className="rounded-xl border border-[color:var(--color-border)] bg-[color:var(--color-surface-2)] p-2.5 text-[color:var(--color-text-muted)] transition-colors group-hover/item:border-brand-terra group-hover/item:text-brand-terra shrink-0">
                    <Clock className="h-5 w-5" />
                  </div>
                  <div className="pt-0.5">
                    <p className="font-heading text-lg text-[color:var(--color-text)] mb-1 group-hover/item:text-brand-terra transition-colors">Conserva el Hilo</p>
                    <p className="text-sm font-light text-[color:var(--color-text-muted)] leading-relaxed">Responde siempre sobre el ticket abierto en lugar de crear uno nuevo.</p>
                  </div>
                </div>
              </div>
            </div>
