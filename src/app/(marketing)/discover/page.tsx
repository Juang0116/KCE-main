  ...
  return (
    <main className="min-h-screen bg-[var(--color-bg)] pb-24 pt-24 md:pt-32">
      <div className="mx-auto max-w-7xl px-6">
        ...
        {/* HERO DISCOVER */}
        <header className="relative mb-16 overflow-hidden rounded-[3.5rem] border border-[var(--color-border)] bg-brand-blue p-10 md:p-20 text-white shadow-2xl">
          ...
          <div className="relative z-10 max-w-4xl">
            ...
            <h1 className="font-heading text-4xl leading-tight md:text-6xl lg:text-7xl mb-8">
              ...
              <span className="text-brand-yellow font-light italic text-3xl md:text-5xl lg:text-6xl">sin perder el foco comercial.</span>
            </h1>
        ...
        <section className="mb-20 grid gap-6 md:grid-cols-3">
          {[
            { icon: Sparkles, title: 'Inspiración Pura', copy: 'Discover es una capa editorial para leer y comparar ideas.', color: 'text-brand-yellow' },
            ...
            <div key={i} className="rounded-[2.5rem] border border-[var(--color-border)] bg-[var(--color-surface)] p-8 shadow-sm transition-all hover:shadow-xl hover:-translate-y-1">
              <card.icon className={`h-8 w-8 ${card.color} mb-6`} />
              <h3 className="font-heading text-xl text-brand-blue mb-3">{card.title}</h3>
              <p className="text-sm font-light text-[var(--color-text)]/70 leading-relaxed">{card.copy}</p>
            </div>
          ))}
        </section>
        ...
        {editorialItems.length > 0 && (
          <section className="space-y-12">
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 border-b border-[var(--color-border)] pb-8">
              <div>
                <p className="text-[10px] font-bold uppercase tracking-[0.2em] text-[var(--color-text)]/40 mb-2">Editorial Picks</p>
            ...
            <div className="grid gap-8 md:grid-cols-2 lg:grid-cols-3">
              {editorialItems.map((it) => {
                ...
                return (
                  <Link key={it.id} href={href} className="group flex flex-col rounded-[2.5rem] border border-[var(--color-border)] bg-[var(--color-surface)] overflow-hidden shadow-sm transition-all hover:shadow-2xl hover:-translate-y-2">
                    <div className="relative h-56 w-full overflow-hidden">
                      {imgSrc ? (
                        ...
                        <div className="flex h-full w-full items-center justify-center bg-[var(--color-surface-2)] text-brand-blue/20">
                    ...
                    <div className="p-8 flex-1 flex flex-col">
                      ...
                      <p className="text-sm font-light text-[var(--color-text)]/60 line-clamp-3 mb-6">
        ...
        <section className="mt-24 rounded-[3.5rem] border border-[var(--color-border)] bg-[var(--color-surface-2)] p-10 md:p-16 shadow-inner text-center">
          <div className="max-w-3xl mx-auto">
            ...
            <p className="text-lg font-light text-[var(--color-text)]/70 leading-relaxed mb-10">
