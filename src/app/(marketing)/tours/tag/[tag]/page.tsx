      ...
      { '@type': 'CollectionPage', name: `Tours: ${tagName}`, url: canonical },
      {
        ...
        itemListElement: [
          { '@type': 'ListItem', position: 1, name: 'Inicio', item: absoluteUrl(`/${locale}`) },
          {
            ...
            position: 2,
            name: 'Estilos',
            item: absoluteUrl(`/${locale}/tours/styles`),
          },
    ...
    <>
      ...
      <main className="min-h-screen animate-fade-in bg-base pb-24">
        ...
        <section className="relative overflow-hidden border-b border-brand-dark/10 bg-brand-dark px-6 py-24 text-center md:py-32">
          ...
          <div className="pointer-events-none absolute left-1/2 top-0 h-80 w-full max-w-4xl -translate-x-1/2 rounded-full bg-brand-terra/10 blur-[120px]" />
          ...
          <div className="relative z-10 mx-auto flex max-w-4xl flex-col items-center">
            ...
            <h1 className="mb-8 font-heading text-5xl leading-[1.05] tracking-tight text-white md:text-7xl lg:text-8xl">
              ...
              <span className="font-light italic text-brand-terra opacity-90">{tagName}</span>.
            </h1>

            <p className="mx-auto max-w-2xl text-lg font-light leading-relaxed text-white/70 md:text-xl">
              Experiencias inmersivas enfocadas en la esencia de{' '}
              <span className="font-medium text-white">{tagName.toLowerCase()}</span>. Encuentra tu
              ruta ideal filtrando por destino.
            </p>

            <div className="mt-10">
              <Link
                href={withLocale(locale, '/tours/styles')}
                className="group flex items-center gap-3 text-[10px] font-bold uppercase tracking-widest text-white/50 transition-colors hover:text-brand-blue"
              >
        ...
        <section className="mx-auto w-full max-w-[var(--container-max)] px-6 py-12">
          ...
          <div className="mb-16 flex flex-col gap-10 border-b border-brand-dark/5 pb-12 dark:border-white/5 lg:flex-row lg:items-end lg:justify-between">
            <div className="w-full max-w-4xl flex-1">
              ...
              {cities?.length > 0 && (
                <div className="mt-8 flex flex-wrap items-center gap-3">
                  <span className="mr-2 text-[10px] font-bold uppercase tracking-widest text-muted">
                    Filtrar por Destino:
                  </span>
                  {cities.slice(0, 10).map((c) => (
                    <Link
                      ...
                      className={`rounded-full border px-5 py-2 text-[10px] font-bold uppercase tracking-widest shadow-sm transition-all duration-300 ${
                        city === c
                          ? 'scale-105 border-brand-blue bg-brand-blue text-white shadow-pop'
                          : 'border-brand-dark/10 bg-surface text-muted hover:border-brand-blue hover:text-brand-blue dark:border-white/10'
                      }`}
                    >
                      {c}
                    </Link>
            ...
            <div className="flex shrink-0 items-center gap-3 rounded-full border border-brand-dark/5 bg-surface-2 px-6 py-3 text-[10px] font-bold uppercase tracking-[0.3em] text-muted shadow-sm">
              <span className="font-heading text-base text-brand-blue">{toursRes.total}</span>{' '}
              Experiencias
            </div>
          </div>

          {/* 03. CARTA DE CONTEXTO (Grid de Beneficios Premium) */}
          <div className="mb-20 border-b border-brand-dark/5 pb-20 dark:border-white/5">
              ...
              {[
                {
                  ...
                  copy: 'Experiencias verificadas en el terreno que comparten la misma intención de calidad KCE.',
                },
                {
                  icon: MapPin,
                  color: 'text-brand-terra',
                  title: 'Filtra por Ciudad',
                  copy: 'Cruza el estilo con el destino que prefieras para aterrizar tu plan ideal en segundos.',
                },
                {
                  icon: Compass,
                  color: 'text-brand-yellow',
                  title: 'Asistencia Humana',
                  copy: '¿No encuentras el tour perfecto? Nuestro concierge puede diseñarlo a medida para ti.',
                },
              ].map((item, idx) => (
                <div
                  key={idx}
                ...
                >
                  <div
                    className={`mb-5 flex h-14 w-14 items-center justify-center rounded-2xl border border-brand-dark/5 bg-surface ${item.color} shadow-soft transition-all duration-500 group-hover:scale-110 group-hover:border-brand-blue/20`}
                  >
          ...
          {toursRes.items.length > 0 ? (
            ...
            <div className="flex flex-col items-center justify-center rounded-[var(--radius-3xl)] border border-brand-dark/5 bg-surface py-32 text-center shadow-soft">
              ...
              <h2 className="mb-4 font-heading text-3xl tracking-tight text-main md:text-4xl">
                Sin resultados para {tagName}
              </h2>
              <p className="mx-auto mb-10 max-w-md px-4 text-base font-light leading-relaxed text-muted">
                Prueba ajustando los filtros de destino o explora otros estilos de viaje en nuestro
                catálogo completo.
              </p>
        ...
        <section className="mx-auto mb-12 mt-16 w-full max-w-[var(--container-max)] px-6">
          <div className="group relative overflow-hidden rounded-[var(--radius-3xl)] border border-brand-dark/5 bg-surface p-12 text-center shadow-soft dark:border-white/5 md:p-24">
            ...
            <div className="relative z-10 mx-auto flex max-w-3xl flex-col items-center">
              ...
              <p className="mb-12 text-lg font-light leading-relaxed text-muted md:text-xl">
                Nuestros expertos pueden diseñar una expedición privada que combine tus estilos
                favoritos en una sola ruta de autor.
              </p>
              <Button
                asChild
                size="lg"
