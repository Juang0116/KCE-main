    ...
    <main className="min-h-screen bg-[var(--color-bg)] flex flex-col animate-fade-in">
      ...
      <div className="w-full bg-[var(--color-surface-2)]/30 border-b border-[var(--color-border)] py-3 px-6">
        <div className="mx-auto max-w-[var(--container-max)] flex items-center justify-center sm:justify-start gap-2 text-[10px] font-bold uppercase tracking-[0.2em] text-[var(--color-text-muted)] opacity-80">
          ...
          <ArrowRight className="h-3 w-3" />
          <span className="text-[var(--color-text)] opacity-50">Planificador</span>
        </div>
      </div>
      ...
      <div className="mx-auto w-full max-w-[var(--container-max)] px-6 py-20 flex flex-col gap-24 flex-1">
        ...
        <section className="grid gap-12 lg:grid-cols-[1fr_380px] items-start">
          ...
          <div className="relative">
            <header className="mb-10 flex items-center justify-between border-b border-[var(--color-border)] pb-8">
              <div className="flex items-center gap-4">
                <div className="flex h-14 w-14 items-center justify-center rounded-2xl bg-[var(--color-surface-2)] border border-[var(--color-border)] text-brand-blue shadow-sm">
                  <Compass className="h-6 w-6" />
                </div>
                <div>
                  <h2 className="font-heading text-3xl text-[var(--color-text)] tracking-tight mb-1">Configura tu experiencia</h2>
                  <p className="text-xs font-bold uppercase tracking-widest text-[var(--color-text-muted)] opacity-70">Paso 1: Estilo y ritmo</p>
                </div>
              </div>
              <ShieldCheck className="hidden sm:block h-8 w-8 text-brand-blue/20" />
            </header>
            
            {/* El formulario respira libremente, eliminamos bg-[var(--color-surface)] y shadow-soft */}
          ...
          <aside className="space-y-8 sticky top-32">
            ...
            <div className="rounded-[var(--radius-2xl)] border border-[var(--color-border)] bg-[var(--color-surface-2)]/50 p-8 shadow-soft">
              <h3 className="font-heading text-2xl text-[var(--color-text)] mb-8 tracking-tight">¿Por qué usar el planificador?</h3>
              <div className="space-y-8">
                {[
                  ...
                  <div key={idx} className="flex items-start gap-4 group">
                    <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-[var(--color-surface)] border border-[var(--color-border)] text-[var(--color-text-muted)] transition-all group-hover:border-brand-blue group-hover:text-brand-blue group-hover:shadow-sm">
                      <item.icon className="h-4 w-4" />
                    </div>
                    <div className="pt-0.5">
                      <h4 className="text-sm font-semibold text-[var(--color-text)] group-hover:text-brand-blue transition-colors mb-1">{item.title}</h4>
            ...
            <div className="relative overflow-hidden rounded-[var(--radius-2xl)] border border-[var(--color-border)] bg-[var(--color-surface)] p-8 text-center shadow-soft group">
              ...
              <div className="relative z-10 flex flex-col items-center">
                <h3 className="font-heading text-xl text-[var(--color-text)] mb-3 tracking-tight">¿Prefieres explorar solo?</h3>
                <p className="text-sm font-light text-[var(--color-text-muted)] mb-8 leading-relaxed">
                  Si ya tienes claro lo que buscas, puedes saltar directamente a nuestro catálogo completo y reservar.
                </p>
                <Button asChild variant="outline" className="w-full rounded-full bg-[var(--color-surface-2)] border-[var(--color-border)] hover:border-brand-blue hover:text-brand-blue transition-colors">
      ...
      <section className="bg-[var(--color-surface-2)]/30 border-t border-[var(--color-border)]">
