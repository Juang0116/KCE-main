        ...
        for (const ch of channels) {
          if (opts.dryRun) {
            r.actions.push({
              ...
              status: 'skipped',
              details: { channel: ch, minutes: 60, reason: alert.message },
            });
            continue;
          }
          ...
          r.actions.push({
            action: 'pause_channel',
            status: 'applied',
            details: { channel: normalized, minutes: 60 },
          });
        }

        // 2) Create internal ticket
        if (!opts.dryRun) {
          const created = await createInternalTicket(
            'Outbound failures spike',
            `${alert.message}
...
Auto-action: channels paused for 60 minutes.`,
          ...
          r.actions.push({
            ...
            status: 'skipped',
            details: { subject: 'Outbound failures spike' },
          });
        }
      }
      ...
      if (alert.type === 'paid_rate_drop') {
        ...
        if (!opts.dryRun) {
          ...
          await ledger(alert, 'disable_auto_promote', 'applied', {
            key: 'crm_auto_promote_weights',
            value: 'false',
          });
          r.actions.push({
            action: 'disable_auto_promote',
            status: 'applied',
            details: { key: 'crm_auto_promote_weights', value: 'false' },
          });
        } else {
          r.actions.push({
            action: 'disable_auto_promote',
...
Auto-action: auto-promote weights disabled (runtime flag). Review checkout UX and templates.`,
          ...
          r.actions.push({
            action: 'create_ticket',
            status: 'skipped',
            details: { subject: 'Paid-rate drop detected' },
          });
        }
      }
    } catch (e) {
