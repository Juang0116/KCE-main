...
- `next.config.ts`
  ...
  - incluye `localhost`, `127.0.0.1`, tu IP LAN actual y `NEXT_PUBLIC_SITE_URL` si corre por HTTP en
    dev.
- `.env.example`
...
- No se cambiaron otras páginas a edge para no sacrificar generación estática o introducir
  regresiones innecesarias.
- La recomendación es mantener Node.js para páginas/handlers con lógica de datos y reservar edge
  para OG/middleware/light request logic.
