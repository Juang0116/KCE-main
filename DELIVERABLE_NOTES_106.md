...
- Added `/api/plan/submit` as an alias to the current quiz submission handler so the new naming can
  be used without breaking the existing CRM/routing logic.
...
- Discover hub was marked `noindex` and removed from the sitemap core route list to reduce premature
  editorial weight in indexing.
...
- Replace remaining internal/public wording like "matcher" / legacy "quiz" terminology where it
  still survives.
