# ADV MAR — Growth / SEO / Landing System Sprint 44

## Objetivo

Subir KCE en adquisición internacional y en estructura de captación, no solo en UI.

## Qué se hizo

- Nuevo componente reusable `InternationalGrowthDeck`
  - rutas premium de adquisición / conversión
  - catálogo
  - matcher
  - lead magnet EU
  - handoff humano
- Integrado en:
  - home
  - tours
  - destinations
  - discover
  - lead magnet EU
- Home:
  - `generateMetadata()` nueva
  - OG / alternates / canonical
  - JSON-LD ampliado con `Organization` + `WebSite` + `SearchAction`
- Discover:
  - página rehecha con locale-awareness
  - metadata propia
  - quick actions más orientadas a demanda calificada
- Lead magnet EU:
  - metadata locale-aware
  - deck de crecimiento para mover research traffic hacia tours / quiz / contacto

## Impacto esperado

- mejor estructura de enlazado interno
- más claridad comercial para tráfico internacional
- mejor base SEO / sharing / discovery
- mejor puente entre inspiración, lead capture y cierre
