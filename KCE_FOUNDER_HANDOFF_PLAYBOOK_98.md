...
1. `ChatWidget` ahora guarda y muestra `lead`, `ticket`, `deal` y `task` cuando el handoff humano
   abre continuidad comercial.
2. `ContactForm` muestra un cierre más empresarial con contexto resumido y rutas claras para
   continuar.
3. `contact/page.tsx` pasa enlaces de continuidad (`tour`, `plan`, `tours`, `WhatsApp/FAQ`) para que
   el usuario siga sin fricción.
