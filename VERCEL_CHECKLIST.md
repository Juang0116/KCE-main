...
**Causa:** Falta `GEMINI_API_KEY` en Vercel. **Fix:** Vercel → Project → Settings → Environment
Variables → Add:
...
**Causa:** La DB de Supabase no tiene tours (SQL patch no fue ejecutado). **Fix:** Supabase → SQL
Editor → pegar y ejecutar `supabase_patch_p93_tours_seed.sql`
...
Para arreglar "Leaked Password Protection Disabled": Supabase → Authentication → Settings → Enable
...
- **autopilot** (cada hora): CRM + Ops Agent (recordatorios pre-tour) + Review Agent (pedidos de
  reseña)
...
- `geminiKeyLength: 40` con prefix `AIzaSy` pero `geminiTestOk: false` → la key está expirada o
  tiene restricciones de dominio en Google AI Studio
- `geminiTestError: fetch failed` → Vercel no puede alcanzar generativelanguage.googleapis.com (raro
  pero posible en algunas regiones)
...
**Paso 4 — Test manual de email:** En Resend → Logs puedes ver si los emails se están intentando
enviar y si fallan.

**Paso 5 — ¿Está el canal pausado?** En Supabase → SQL Editor ejecuta:
