...
Mejorar la experiencia mobile en el header/menu con foco en acceso a cuenta e integración visible de
WhatsApp.
...
- `src/components/Header.tsx`
  ...
  - `MobileAuthActions` en header mobile usa modo `compact` para que siempre haya un acceso visible
    a login/cuenta.
...
- `src/features/auth/AuthMenu.tsx`
  - Estado signed-out en mobile ahora renderiza una tarjeta clara de Cuenta con CTA principal
    `Iniciar sesión` y CTA secundaria `Crear cuenta`.
