...
- `/admin/sales` actions (Copy/WhatsApp/Email) now request templates from DB and fallback to local
  playbook if missing.
- `/admin/tickets/[id]` quick templates now use DB keys (`ticket.first_reply`,
  `ticket.followup_24h`, etc.) and fallback to local.
