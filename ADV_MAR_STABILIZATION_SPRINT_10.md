...
- Las tablas runtime/Admin/Ops/CRM (`affiliate_clicks`, `ops_incidents`, etc.) estaban definidas en
  el archivo, pero habían quedado fuera de `Database['public']['Tables']` por un cierre de llave
  incorrecto.
- Se movieron efectivamente dentro del bloque `Tables`, que es el que usa Supabase para tipar
  `.from('...')`.

## Impacto

- `affiliate_clicks` ya entra en el union literal de `.from(...)`.
- `ops_incidents` y otras tablas runtime quedan tipadas de verdad, no solo “presentes en el
  archivo”.
