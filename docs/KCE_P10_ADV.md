...
- **Checkout sin pago**: si un deal está en `checkout` y no se ha cerrado (`closed_at IS NULL`) y
  `updated_at` es mayor a `CRM_CHECKOUT_UNPAID_AFTER_HOURS` (default 2h), se encola un recordatorio.
- **Ticket esperando cliente**: si un ticket está `open/pending/in_progress` y la conversación está
  esperando respuesta del cliente por más de `CRM_WAITING_CUSTOMER_AFTER_HOURS` (default 24h), se
  encola un ping (usa el deal activo del lead/cliente).
...
- No se encola si hubo un mensaje `queued/sending/sent` en las últimas
  `CRM_OUTBOUND_MIN_INTERVAL_HOURS` (default 8h).
- Máximo `CRM_OUTBOUND_MAX_PER_7D` (default 3) por deal+canal en 7 días.

### 3) Enqueue por cambio de etapa (stage transition)

Cuando un admin cambia `stage` en un deal (`PATCH /api/admin/deals/[id]`), se encola (best-effort)
un mensaje para etapas:
