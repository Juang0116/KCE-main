...
`findLatestIncidentByKind()` ahora usa `getSupabaseAdminAny()` para tablas Ops no alineadas todavía
en `src/types/supabase.ts`.

### 3) exactOptionalPropertyTypes endurecido

El helper `add(...)` ahora limpia `detail` y `meta` antes de empujar al arreglo `checks`, evitando
futuros errores por `undefined` explícito.
